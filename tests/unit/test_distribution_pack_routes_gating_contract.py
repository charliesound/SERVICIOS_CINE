"""
Contract test: distribution_pack_routes.py gating invariants.

Verifies that distribution pack endpoints require tenant context,
...
TARGET_FILE = ROUTES_DIR / "distribution_pack_routes.py"
...
def test_all_project_endpoints_have_validate_project_access():
    ...
    for decorator in decorators:
        ...
        assert "validate_project_access" in block, (
            f"Project-scoped distribution pack endpoint missing validate_project_access: {decorator}"
        )
...
def test_all_write_endpoints_have_require_write_permission():
    ...
    for decorator in decorators:
        ...
        assert "require_write_permission" in block, (
            f"Mutating distribution pack endpoint missing require_write_permission: {decorator}"
        )
...
def test_inline_project_lookup_removed():
    source = _read_source()
