...
def _apply_env_overrides(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    llm_overrides = {
        ...
        "script_analysis_model": ("OLLAMA_SCRIPT_ANALYSIS_MODEL", "qwen2.5:32b"),
        "analysis_model": ("OLLAMA_ANALYSIS_MODEL", os.getenv("OLLAMA_SCRIPT_ANALYSIS_MODEL", "qwen2.5:32b")),
        "storyboard_prompt_model": ("OLLAMA_STORYBOARD_PROMPT_MODEL", "qwen2.5:14b"),
        "pipeline_builder_model": ("OLLAMA_PIPELINE_BUILDER_MODEL", "qwen2.5:14b"),
        "quick_model": ("OLLAMA_QUICK_MODEL", "qwen2.5:7b"),
        "visual_model": ("OLLAMA_VISUAL_MODEL", "qwen2.5:7b"),
        "fallback_model": ("OLLAMA_FALLBACK_MODEL", "mistral:7b"),
        "smoke_model": ("OLLAMA_SMOKE_MODEL", "tinyllama"),
        "keep_alive": ("OLLAMA_KEEP_ALIVE", "30m"),
        "timeout_seconds": ("LLM_TIMEOUT_SECONDS", 240),
...
def get_llm_settings() -> Dict[str, Any]:
    ...
    return {
        ...
        "script_analysis_model": str(llm_config.get("script_analysis_model", os.getenv("OLLAMA_SCRIPT_ANALYSIS_MODEL", "qwen2.5:32b"))).strip(),
        "analysis_model": str(
            llm_config.get(
                "analysis_model",
                os.getenv("OLLAMA_ANALYSIS_MODEL", os.getenv("OLLAMA_SCRIPT_ANALYSIS_MODEL", "qwen2.5:32b")),
            )
        ).strip(),
        "storyboard_prompt_model": str(llm_config.get("storyboard_prompt_model", os.getenv("OLLAMA_STORYBOARD_PROMPT_MODEL", "qwen2.5:14b"))).strip(),
        "pipeline_builder_model": str(llm_config.get("pipeline_builder_model", os.getenv("OLLAMA_PIPELINE_BUILDER_MODEL", "qwen2.5:14b"))).strip(),
        "quick_model": str(llm_config.get("quick_model", os.getenv("OLLAMA_QUICK_MODEL", "qwen2.5:7b"))).strip(),
        "visual_model": str(llm_config.get("visual_model", os.getenv("OLLAMA_VISUAL_MODEL", "qwen2.5:7b"))).strip(),
        "fallback_model": str(llm_config.get("fallback_model", os.getenv("OLLAMA_FALLBACK_MODEL", "mistral:7b"))).strip(),
        "smoke_model": str(llm_config.get("smoke_model", os.getenv("OLLAMA_SMOKE_MODEL", "tinyllama"))).strip(),
        "keep_alive": str(llm_config.get("keep_alive", os.getenv("OLLAMA_KEEP_ALIVE", "30m"))).strip(),
        "timeout_seconds": int(llm_config.get("timeout_seconds", os.getenv("LLM_TIMEOUT_SECONDS", 240))),
