...
DOC_PATH = REPO_ROOT / "docs/product/local_media_agent/cid_local_media_agent_controlled_local_demo_runner_installed_cli_qa_gate_v1.md"
...
PHASE = (
    ...
    "CONTROLLED.LOCAL.DEMO.RUNNER.INSTALLED.CLI.QA.GATE.V1"
)
...
EXPECTED_RESULT = "LOCAL_MEDIA_AGENT_CONTROLLED_LOCAL_DEMO_RUNNER_INSTALLED_CLI_QA_GATE_V1_CLOSED"
...
def _load_runner_json(*extra_args: str) -> dict:
...
def _assert_common_verified_payload(payload: dict) -> None:
    assert payload["status"] == EXPECTED_STATUS
    assert payload["operational_boundary"] == EXPECTED_BOUNDARY
    assert payload["runner_name"] == "cid-local-media-agent-visible-report-write-enabled-export-controlled-local-demo-runner"
    assert payload["command_name"] == CURRENT_COMMAND
    ...
    assert payload["dry_run_exit_code"] == 0
    assert payload["write_exit_code"] == 0
    assert payload["negative_exit_code"] == 1
...
def test_installed_cli_qa_gate_doc_freezes_expected_contract() -> None:
...
def test_installed_runner_help_exposes_controlled_surface() -> None:
    ...
    assert "client-facing" in result.stdout
...
def test_installed_runner_default_result_json_cleans_output_root() -> None:
    payload = _load_runner_json()

    _assert_common_verified_payload(payload)
...
def test_installed_runner_keep_output_preserves_single_artifact_and_test_cleans_it() -> None:
    payload = _load_runner_json("--keep-output")
    ...
    try:
        ...
        assert artifact_path == Path(payload["write"]["artifact_path"])
        ...
        txt_artifacts = list(root.rglob("*.txt"))
        assert txt_artifacts == [artifact_path]
