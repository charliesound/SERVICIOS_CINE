...
DOC = ROOT / "docs/product/local_media_agent/cid_local_media_agent_real_media_preflight_isolated_implementation_gate_v1.md"
...
def test_isolated_implementation_gate_doc_exists():
    assert DOC.exists()


def test_phase_identifier_is_present():
    text = _text()
    assert "CID.LOCAL_MEDIA_AGENT.REAL_MEDIA_PREFLIGHT.ISOLATED_IMPLEMENTATION.GATE.V1" in text


def test_expected_closure_result_is_present():
    text = _text()
    assert "LOCAL_MEDIA_AGENT_REAL_MEDIA_PREFLIGHT_ISOLATED_IMPLEMENTATION_GATE_V1_CLOSED" in text


def test_starting_state_is_ready_for_isolated_implementation_gate():
    text = _text()
    assert "READY_FOR_ISOLATED_IMPLEMENTATION_GATE" in text


def test_target_state_prepares_code_skeleton_readiness():
    text = _text()
    assert "ISOLATED_IMPLEMENTATION_BOUNDARY_READY_FOR_CODE_SKELETON_READINESS_GATE" in text


def test_gate_purpose_defines_boundary_only():
    text = _text()
    assert "This gate defines the boundary for a later isolated code skeleton readiness gate." in text
    assert "This gate creates only a sanitized isolated implementation boundary record." in text
    assert "This gate does not create runtime implementation." in text
    assert "This gate does not create a code skeleton." in text
    assert "This gate is limited to documentation and tests." in text


def test_source_isolated_implementation_readiness_gate_is_referenced():
    text = _text()
    assert "CID.LOCAL_MEDIA_AGENT.REAL_MEDIA_PREFLIGHT.ISOLATED_IMPLEMENTATION.READINESS.GATE.V1" in text
    assert "LOCAL_MEDIA_AGENT_REAL_MEDIA_PREFLIGHT_ISOLATED_IMPLEMENTATION_READINESS_GATE_V1_CLOSED" in text


def test_source_isolated_implementation_readiness_record_values_are_preserved():
    text = _text()
    required_values = [
        "isolated_implementation_readiness_001",
        "operator_input_001",
        ...
        "single_file_claimed",
        "ready_for_isolated_implementation_gate_without_runtime_stat_or_metadata_read",
    ]
    for value in required_values:
        assert value in text


def test_isolated_implementation_boundary_record_is_declared():
    text = _text()
    required_fields = [
        "ISOLATED_IMPLEMENTATION_BOUNDARY_RECORD_ID",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_INPUT_RECORD_ID",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_SOURCE_READINESS_RECORD_ID",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_SOURCE_REAL_STAT_IMPLEMENTATION_CONTRACT_RECORD_ID",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_SOURCE_REAL_STAT_IMPLEMENTATION_CONTRACT_HANDLE",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_SOURCE_STAT_EXECUTION_BOUNDARY_RECORD_ID",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_SOURCE_STAT_EXECUTION_BOUNDARY_HANDLE",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_SOURCE_CONTROLLED_STAT_BOUNDARY_RECORD_ID",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_SOURCE_CONTROLLED_STAT_BOUNDARY_HANDLE",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_SOURCE_REAL_FILE_ACCESS_BOUNDARY_RECORD_ID",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_SOURCE_REAL_FILE_ACCESS_BOUNDARY_HANDLE",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_SOURCE_LOCAL_PATH_DISCLOSURE_BOUNDARY_RECORD_ID",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_SOURCE_LOCAL_PATH_DISCLOSURE_BOUNDARY_HANDLE",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_SOURCE_SELECTION_BOUNDARY_RECORD_ID",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_SOURCE_SELECTION_BOUNDARY_HANDLE",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_SOURCE_CONFIRMATION_RECORD_ID",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_SOURCE_CONFIRMATION_HANDLE",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_SOURCE_CONFIRMATION_VALUE",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_SOURCE_TOKEN_RECORD_ID",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_SOURCE_TOKEN_HANDLE",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_SANITIZED_SELECTION_TOKEN",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_SOURCE_REFERENCE_RECORD_ID",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_SOURCE_REFERENCE_HANDLE",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_SOURCE_EVENT_ID",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_SOURCE_EVENT_HANDLE",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_HANDLE",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_VERDICT",
    ]
    for field in required_fields:
        assert field in text


def test_isolated_implementation_boundary_values_are_sanitized():
    text = _text()
    required_values = [
        "isolated_implementation_boundary_001",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_HANDLE_001",
        "defined_as_isolated_implementation_boundary",
        "prepares_code_skeleton_readiness_only",
        "boundary_defined_without_runtime_creation",
        "not_created_in_this_gate",
        "isolated_implementation_boundary_defined_without_code_runtime_stat_or_metadata_read",
    ]
    for value in required_values:
        assert value in text
...
def test_boundary_status_and_scope_are_limited():
    text = _text()
    assert "ISOLATED_IMPLEMENTATION_BOUNDARY_STATUS" in text
    assert "defined_as_isolated_implementation_boundary" in text
    assert "ISOLATED_IMPLEMENTATION_BOUNDARY_SCOPE_STATUS" in text
    assert "prepares_code_skeleton_readiness_only" in text
    assert "The boundary status is `defined_as_isolated_implementation_boundary`." in text
    assert "The boundary scope prepares code skeleton readiness only." in text


def test_no_code_skeleton_runtime_or_cli_modification_occurs():
    text = _text()
    required_statuses = [
        "ISOLATED_IMPLEMENTATION_BOUNDARY_CODE_SKELETON_STATUS",
        "not_created_in_this_gate",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_RUNTIME_STATUS",
        "no_runtime_created",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_CLI_RUNTIME_STATUS",
        "not_modified",
    ]
    for status in required_statuses:
        assert status in text
    assert "No code skeleton is created in this gate." in text
...
def test_no_stat_execution_occurs():
    text = _text()
    assert "ISOLATED_IMPLEMENTATION_BOUNDARY_STAT_STATUS" in text
...
def test_no_real_file_access_occurs():
    text = _text()
    assert "ISOLATED_IMPLEMENTATION_BOUNDARY_ACCESS_STATUS" in text
...
def test_no_media_file_is_opened():
    text = _text()
    assert "ISOLATED_IMPLEMENTATION_BOUNDARY_FILE_OPEN_STATUS" in text
...
def test_no_file_bytes_or_real_metadata_are_read():
    text = _text()
    required_statuses = [
        "ISOLATED_IMPLEMENTATION_BOUNDARY_FILE_BYTES_STATUS",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_FILESYSTEM_METADATA_STATUS",
        "not_read",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_file_size_timestamps_or_hashes_are_recorded():
    text = _text()
    required_statuses = [
        "ISOLATED_IMPLEMENTATION_BOUNDARY_FILE_SIZE_STATUS",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_TIMESTAMP_STATUS",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_HASH_STATUS",
        "not_recorded",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_local_path_or_identity_metadata_is_committed():
    text = _text()
    required_statuses = [
        "ISOLATED_IMPLEMENTATION_BOUNDARY_REAL_PATH_STATUS",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_ABSOLUTE_PATH_STATUS",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_RELATIVE_PATH_STATUS",
        "not_recorded_in_committed_artifacts",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_REAL_FILENAME_STATUS",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_PARENT_FOLDER_STATUS",
        "not_recorded",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_media_decode_probe_scan_or_transcription_occurs():
    text = _text()
    required_statuses = [
        "ISOLATED_IMPLEMENTATION_BOUNDARY_MEDIA_DECODE_STATUS",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_MEDIA_PROBE_STATUS",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_MEDIA_SCAN_STATUS",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_TRANSCRIPTION_STATUS",
        "not_executed",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_thumbnails_or_waveforms_are_generated():
    text = _text()
    required_statuses = [
        "ISOLATED_IMPLEMENTATION_BOUNDARY_THUMBNAIL_STATUS",
        "not_generated",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_WAVEFORM_STATUS",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_execution_or_media_tools_are_run():
    text = _text()
    required_statuses = [
        "ISOLATED_IMPLEMENTATION_BOUNDARY_EXECUTION_STATUS",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_FFMPEG_STATUS",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_FFPROBE_STATUS",
        "ISOLATED_IMPLEMENTATION_BOUNDARY_SCANNER_STATUS",
        "not_executed",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_saas_integration_is_created():
    text = _text()
    assert "ISOLATED_IMPLEMENTATION_BOUNDARY_SAAS_STATUS" in text
    assert "no_saas_integration" in text
    assert "No SaaS integration is created." in text


def test_code_skeleton_readiness_constraints_are_conservative():
    text = _text()
    constraints = [
        ...
        "It must define code skeleton conditions without executing against a real file.",
...
def test_positive_assertions_preserve_scope():
    ...
    required_assertions = [
        ...
        "`isolated_implementation_boundary_001` is created as a sanitized boundary record.",
        "`ISOLATED_IMPLEMENTATION_BOUNDARY_HANDLE_001` is a non-filesystem boundary handle.",
        "`isolated_implementation_readiness_001` remains the source readiness record.",
...
def test_code_skeleton_runtime_and_stat_execution_are_forbidden():
    text = _text()
    forbidden_items = [
        "Creating runtime implementation.",
        "Creating code skeleton files.",
...
def test_next_phase_boundary_is_code_skeleton_readiness_only():
    text = _text()
    assert "The next conservative phase may prepare a code skeleton readiness gate." in text
    assert "This isolated implementation gate does not authorize code skeleton creation." in text
    assert "This isolated implementation gate does not authorize runtime implementation." in text
    assert "This isolated implementation gate does not authorize filesystem stat execution." in text
    assert "This isolated implementation gate does not authorize accessing a real file." in text
    assert "This isolated implementation gate does not authorize opening media." in text
    assert "This isolated implementation gate does not authorize reading file bytes." in text
    assert "This isolated implementation gate does not authorize reading real metadata." in text
    assert "This isolated implementation gate does not authorize media execution." in text
    assert "This isolated implementation gate only defines the isolated implementation boundary." in text


def test_required_checks_reference_previous_gates_and_generic_guards():
    text = _text()
    required_checks = [
        "This isolated implementation gate test.",
...
def test_closing_state_is_ready_for_code_skeleton_readiness_gate():
    text = _text()
    assert "ISOLATED_IMPLEMENTATION_BOUNDARY_READY_FOR_CODE_SKELETON_READINESS_GATE" in text
