"""
Contract test: sales_targets_routes.py gating invariants.

Verifies that sales targets endpoints require tenant context,
...
TARGET_FILE = ROUTES_DIR / "sales_targets_routes.py"

MANUAL_REVIEW_ENDPOINTS = [
    # target_id validated by tenant org_id within services.
    # opportunity_id validated by project_id within services.
]
...
def test_all_project_endpoints_have_validate_project_access():
    source = _read_source()
    decorators = re.findall(r"@router\.[a-z]+\(\s*\"/projects/\{project_id\}[^\"]*\"", source)
    assert len(decorators) == 4, f"Expected 4 project endpoints, found {len(decorators)}"
    for decorator in decorators:
        block = _get_function_block(source, decorator)
...
def test_mutating_endpoints_have_require_write_permission():
    ...
    action_decorators = [
        '"/sales-targets"', # POST
        '"/sales-targets/{target_id}"', # PATCH
        '"/projects/{project_id}/sales-opportunities/suggest"', # POST
        '"/projects/{project_id}/sales-opportunities"', # POST
        '"/projects/{project_id}/sales-opportunities/{opportunity_id}"', # PATCH
    ]
