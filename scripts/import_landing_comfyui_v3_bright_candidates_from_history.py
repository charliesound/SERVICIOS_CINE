...
MANIFEST_PATH = CANDIDATES_DIR / "_landing_v3_bright_candidates_manifest.txt"
...
def next_candidate_path(image_key: str) -> Path:
    index = 1
    while True:
        path = CANDIDATES_DIR / f"candidate-{image_key}-bright-{index:02d}.webp"
        if not path.exists():
            return path
        index += 1
...
def main() -> int:
    if not JOBS_PATH.exists():
        raise FileNotFoundError(f"Missing bright render jobs file: {JOBS_PATH}")
