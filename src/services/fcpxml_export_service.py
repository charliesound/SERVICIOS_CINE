...
class FCPXMLExportService:
    def build_fcpxml(
        ...
        *,
        ...
        ET.SubElement(resources, "format", id="r1", name=f"FFVideoFormat{int(round(fps))}p", frameDuration=self._frame_duration(fps), width="1920", height="1080")
        ...
        for item in items:
            ...
            if audio_asset_id and audio_asset_id != source_asset_id:
                audio_resolution = resolved_assets.get(audio_asset_id, {})
                audio_note = f"external audio: {audio_resolution.get('filename') or audio_asset_id}; dual_system_audio_export_partial"
            if note:
        ...
        file_name = f"{project_name.replace(' ', '_')}_assembly.fcpxml"
        manifest = {
            ...
            "missing_media_count": sum(1 for item in resolved_assets.values() if item.get("status") == "missing"),
            "warnings": warnings,
        }
        return xml_bytes, file_name, manifest
...
fcpxml_export_service = FCPXMLExportService()
