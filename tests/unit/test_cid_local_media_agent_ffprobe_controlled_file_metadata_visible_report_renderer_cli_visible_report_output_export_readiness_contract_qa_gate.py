...
QA_DOC = Path(
    ...
    "cid_local_media_agent_ffprobe_controlled_file_metadata_visible_report_renderer_cli_visible_report_output_export_readiness_contract_qa_gate_v1.md"
)
CONTRACT_DOC = Path(
    "docs/product/local_media_agent/"
...
FUNCTIONAL_RESULT = (
    ...
    "VISIBLE_REPORT_OUTPUT_EXPORT_READINESS_CONTRACT_QA_GATE_PASS_CLOSED"
)
TARGET_TAG = (
    "cid-dev-stable-local-media-agent-ffprobe-controlled-file-metadata-visible-report-"
    "renderer-cli-visible-report-output-export-readiness-contract-qa-gate-v1-20260622"
)
NEXT_PHASE = (
    "CID.LOCAL_MEDIA_AGENT.FFPROBE.CONTROLLED.FILE.METADATA.PREFLIGHT."
...
def test_qa_gate_declares_no_export_no_packaging() -> None:
    assert_all_present(read(QA_DOC), [
...
def test_contract_doc_declares_export_readiness_conditions() -> None:
    assert_all_present(read(CONTRACT_DOC), [
        ...
        "deterministic from controlled JSON",
        "text-only",
        "human-readable",
        "no path leakage",
        "no secrets leakage",
