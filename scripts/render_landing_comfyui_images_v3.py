...
PAYLOADS_DIR = ROOT / ".tmp" / "landing_comfyui_v3" / "image_payloads"
JOBS_PATH = ROOT / ".tmp" / "landing_comfyui_v3" / "image_render_jobs.json"
...
def submit_prompt(workflow: dict[str, Any]) -> dict[str, Any]:
    body = json.dumps(
        {
            "prompt": workflow,
            "client_id": f"landing-v3-images-{uuid.uuid4().hex}",
        }
    ).encode("utf-8")
...
def main() -> int:
    if os.environ.get("ENABLE_REAL_COMFYUI_RENDER") != "1":
        print("Render real de imágenes bloqueado. Usa ENABLE_REAL_COMFYUI_RENDER=1.")
        return 0

    payloads = load_payloads()
    if not payloads:
        raise RuntimeError("No image payloads found. Run scripts/build_landing_comfyui_payloads_v3.py first.")
    ...
    print(f"Image render jobs saved to {JOBS_PATH.relative_to(ROOT)}")
