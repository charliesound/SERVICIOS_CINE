import importlib.util
import json
from pathlib import Path


QA_DOC = Path(
    "docs/product/local_media_agent/"
    "cid_local_media_agent_real_preflight_minimal_runtime_cli_real_folder_second_controlled_scenario_"
    "product_semantics_bounded_implementation_qa_gate_v1.md"
)

IMPLEMENTATION_DOC = Path(
    "docs/product/local_media_agent/"
    "cid_local_media_agent_real_preflight_minimal_runtime_cli_real_folder_second_controlled_scenario_"
    "product_semantics_bounded_implementation_v1.md"
)

AUTHORIZATION_DOC = Path(
    "docs/product/local_media_agent/"
    "cid_local_media_agent_real_preflight_minimal_runtime_cli_real_folder_second_controlled_scenario_"
    "product_semantics_implementation_authorization_gate_v1.md"
)

IMPLEMENTATION_TEST = Path(
    "tests/unit/"
    "test_cid_local_media_agent_real_preflight_minimal_runtime_cli_real_folder_second_controlled_scenario_"
    "product_semantics_bounded_implementation.py"
)

SCRIPT = Path("scripts/cid_media_agent_scan.py")

PHASE = "CID.LOCAL_MEDIA_AGENT.REAL.PREFLIGHT.MINIMAL.RUNTIME.CLI.REAL_FOLDER.SECOND.CONTROLLED.SCENARIO.PRODUCT.SEMANTICS.BOUNDED.IMPLEMENTATION.QA.GATE.V1"


def read(path: Path) -> str:
    assert path.exists()
    return path.read_text(encoding="utf-8")


def _load_module():
    spec = importlib.util.spec_from_file_location("cid_media_agent_scan_qa_gate", SCRIPT)
    assert spec is not None
    assert spec.loader is not None
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


def _run_scan(input_root: Path, output_root: Path, *extra: str):
    module = _load_module()
    parser = module.build_parser()
    args = parser.parse_args([
        "--input-root",
        str(input_root),
        "--output-root",
        str(output_root),
        *extra,
    ])
    return module.scan(args)


def _make_second_controlled_shape(root: Path) -> None:
    (root / "camera").mkdir(parents=True)
    (root / "sound").mkdir(parents=True)
    (root / "non_media").mkdir(parents=True)
    (root / "camera" / "A001_SC001_TK001.mov").write_text("placeholder", encoding="utf-8")
    (root / "sound" / "A001_SC001_TK001.wav").write_text("placeholder", encoding="utf-8")
    (root / "non_media" / "notes.txt").write_text("placeholder", encoding="utf-8")
    (root / "non_media" / "tool.exe").write_text("placeholder", encoding="utf-8")


def test_qa_doc_exists_and_names_phase():
    text = read(QA_DOC)
    assert PHASE in text
    assert "Product Semantics Bounded Implementation QA Gate v1" in text


def test_qa_gate_depends_on_bounded_implementation_phase():
    qa_text = read(QA_DOC)
    impl_text = read(IMPLEMENTATION_DOC)

    assert "PRODUCT.SEMANTICS.BOUNDED.IMPLEMENTATION.V1" in qa_text
    assert "PRODUCT_SEMANTICS_BOUNDED_IMPLEMENTATION_PASS_NON_MEDIA_REJECTED" in qa_text
    assert "PRODUCT_SEMANTICS_BOUNDED_IMPLEMENTATION_PASS_NON_MEDIA_REJECTED" in impl_text


def test_qa_gate_depends_on_prior_authorization_gate():
    qa_text = read(QA_DOC)
    auth_text = read(AUTHORIZATION_DOC)

    assert "PRODUCT.SEMANTICS.IMPLEMENTATION.AUTHORIZATION.GATE.V1" in qa_text
    assert "PRODUCT_SEMANTICS_IMPLEMENTATION_AUTHORIZATION_GATE_PASS_SEPARATE_BOUNDED_IMPLEMENTATION_PHASE_AUTHORIZED" in qa_text
    assert "PRODUCT_SEMANTICS_IMPLEMENTATION_AUTHORIZATION_GATE_PASS_SEPARATE_BOUNDED_IMPLEMENTATION_PHASE_AUTHORIZED" in auth_text


def test_qa_result_is_declared_pass():
    text = read(QA_DOC)
    assert "PRODUCT_SEMANTICS_BOUNDED_IMPLEMENTATION_QA_GATE_PASS" in text
    assert "`PASS`" in text


def test_script_preserves_non_media_rejected_extension_policy():
    source = read(SCRIPT)
    assert 'REPORT_EXTENSIONS = {".json", ".csv"}' in source
    assert 'NON_MEDIA_REJECTED_EXTENSIONS = {".txt", ".exe"}' in source
    assert "ignored_extension_counts" in source
    assert "rejected_extension_counts" in source
    assert "accepted_extension_counts" in source


def test_second_controlled_shape_dry_run_has_expected_semantic_counts(tmp_path):
    input_root = tmp_path / "input"
    output_root = tmp_path / "output"
    _make_second_controlled_shape(input_root)

    exit_code, summary = _run_scan(input_root, output_root, "--dry-run")

    assert exit_code == 0
    assert summary["candidate_media_count"] == 2
    assert summary["accepted_extension_counts"] == {".mov": 1, ".wav": 1}
    assert summary["rejected_extension_counts"] == {".exe": 1, ".txt": 1}
    assert summary["ignored_extension_counts"] == {}
    assert summary["created_outputs"] == []
    assert not output_root.exists()


def test_second_controlled_shape_written_status_has_expected_semantic_counts(tmp_path):
    input_root = tmp_path / "input"
    output_root = tmp_path / "output"
    _make_second_controlled_shape(input_root)

    exit_code, summary = _run_scan(input_root, output_root)

    assert exit_code == 0
    assert summary["accepted_extension_counts"] == {".mov": 1, ".wav": 1}
    assert summary["rejected_extension_counts"] == {".exe": 1, ".txt": 1}
    assert summary["ignored_extension_counts"] == {}

    status = json.loads((output_root / "00_project/processing_status.json").read_text())
    assert status["accepted_extension_counts"] == {".mov": 1, ".wav": 1}
    assert status["rejected_extension_counts"] == {".exe": 1, ".txt": 1}
    assert status["ignored_extension_counts"] == {}


def test_rejected_non_media_files_are_excluded_from_media_catalog(tmp_path):
    input_root = tmp_path / "input"
    output_root = tmp_path / "output"
    _make_second_controlled_shape(input_root)

    exit_code, _summary = _run_scan(input_root, output_root)

    assert exit_code == 0
    catalog = json.loads((output_root / "01_media_catalog/media_catalog.json").read_text())
    file_names = {asset["file_name"] for asset in catalog["assets"]}
    extensions = {asset["extension"] for asset in catalog["assets"]}

    assert file_names == {"A001_SC001_TK001.mov", "A001_SC001_TK001.wav"}
    assert extensions == {".mov", ".wav"}


def test_unknown_synthetic_placeholder_txt_legacy_behavior_is_preserved(tmp_path):
    input_root = tmp_path / "input"
    output_root = tmp_path / "output"
    (input_root / "UNKNOWN").mkdir(parents=True)
    (input_root / "UNKNOWN" / "UNKNOWN_ASSET.txt").write_text("placeholder", encoding="utf-8")

    exit_code, summary = _run_scan(input_root, output_root, "--dry-run")

    assert exit_code == 1
    assert summary["candidate_media_count"] == 1
    assert summary["human_review_required_count"] == 1
    assert summary["warnings"] == ["unknown synthetic placeholder"]
    assert summary["rejected_extension_counts"] == {".txt": 1}
    assert summary["ignored_extension_counts"] == {}


def test_implementation_test_contains_required_behavior_coverage():
    text = read(IMPLEMENTATION_TEST)
    for required in [
        "test_dry_run_reports_non_media_rejected_counts_without_outputs",
        "test_written_processing_status_preserves_semantic_counts",
        "test_media_catalog_excludes_rejected_non_media_files",
        "test_unknown_synthetic_placeholder_txt_still_requires_human_review",
        "test_ffprobe_remains_preflight_only_and_not_required_by_default",
    ]:
        assert required in text


def test_qa_doc_records_validation_evidence():
    text = read(QA_DOC)
    for required in [
        "bounded implementation tests: `10 PASS`",
        "scanner safe baseline: `11 PASS`",
        "scanner execution hardening: `15 PASS`",
        "scanner CLI contract: `13 PASS`",
        "product semantics authorization chain: `PASS`",
        "wider local media agent real preflight contracts: `PASS`",
        "`py_compile`: `PASS`",
        "`git diff --check`: `PASS`",
        "`guard_wsl_repo.sh`: `PASS`",
        "`guard_no_sqlite_regressions.sh`: `PASS`",
    ]:
        assert required in text


def test_runtime_boundary_is_limited_to_scanner_file():
    text = read(QA_DOC)
    assert "`scripts/cid_media_agent_scan.py`" in text
    assert "No runtime code is changed by this QA gate." in text
    assert "This QA gate does not authorize any further runtime changes." in text


def test_claim_boundary_remains_blocked():
    text = read(QA_DOC)
    for blocked in [
        "clean classification PASS claims",
        "client-facing classification claims",
        "commercial/client-facing claims",
        "production-ready claims",
    ]:
        assert blocked in text


def test_execution_boundary_remains_blocked():
    text = read(QA_DOC)
    for blocked in [
        "ffmpeg execution",
        "ffprobe execution beyond existing availability preflight",
        "media probing",
        "media decoding",
        "transcription",
        "translation",
        "subtitles",
        "sync",
        "NLE export",
        "SaaS calls",
        "database writes",
        "network calls",
    ]:
        assert blocked in text


def test_protected_scope_remains_blocked():
    text = read(QA_DOC)
    for blocked in [
        "SaaS backend changes",
        "SaaS frontend changes",
        "database changes",
        "Docker changes",
        "Alembic changes",
        "Stripe changes",
        "AI Jobs changes",
        "credits changes",
        "ledger changes",
        "media-processing implementation",
    ]:
        assert blocked in text


def test_script_does_not_emit_client_facing_or_clean_pass_claims():
    source = read(SCRIPT).lower()
    assert "clean classification pass" not in source
    assert "client-facing classification" not in source
    assert "production-ready" not in source


def test_script_does_not_introduce_forbidden_runtime_dependencies():
    source = read(SCRIPT).lower()
    for forbidden in [
        "subprocess",
        "requests",
        "httpx",
        "urllib",
        "socket",
        "sqlalchemy",
        "alembic",
        "stripe",
        "ffmpeg",
        "fastapi",
        "docker",
    ]:
        assert forbidden not in source


def test_ffprobe_remains_availability_preflight_only():
    source = read(SCRIPT)
    assert 'shutil.which("ffprobe")' in source
    assert "subprocess" not in source.lower()


def test_qa_gate_is_docs_test_only():
    text = read(QA_DOC)
    assert "This phase is docs/test-only." in text
    assert "No runtime code is changed by this QA gate." in text


def test_no_broader_scope_is_authorized():
    text = read(QA_DOC)
    assert "No SaaS, database, infrastructure, media-processing, transcription, subtitles, sync, NLE, or report-expansion scope is authorized." in text
    assert "No client-facing clean classification claim is authorized." in text
