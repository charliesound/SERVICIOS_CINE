...
def submit_prompt(workflow: dict[str, Any]) -> dict[str, Any]:
    body = json.dumps(
        {
            "prompt": workflow,
            "client_id": f"landing-v3-strict-{uuid.uuid4().hex}",
        }
    ).encode("utf-8")
...
def main() -> int:
    if os.environ.get("ENABLE_REAL_COMFYUI_RENDER") != "1":
        print("Render real de imagenes strict bloqueado. Usa ENABLE_REAL_COMFYUI_RENDER=1.")
        return 0

    payloads = load_payloads()
    if not payloads:
        raise RuntimeError("No strict subset payloads found. Run scripts/build_landing_comfyui_payloads_v3_strict_subset.py first.")
    ...
    print(f"Strict image render jobs saved to {JOBS_PATH.relative_to(ROOT)}")
