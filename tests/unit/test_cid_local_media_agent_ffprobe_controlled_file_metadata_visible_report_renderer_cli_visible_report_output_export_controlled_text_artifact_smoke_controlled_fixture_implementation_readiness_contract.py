...
TARGET_TAG = (
    ...
    "fixture-implementation-readiness-contract-v1-20260622"
)
NEXT_PHASE = (
    "CID.LOCAL_MEDIA_AGENT.FFPROBE.CONTROLLED.FILE.METADATA.PREFLIGHT."
...
def test_doc_declares_pre_implementation_contract_only_no_runtime() -> None:
    assert_all_present(read(DOC), [
...
def test_doc_does_not_authorize_blocked() -> None:
    assert_all_present(read(DOC), [
        ...
        "does not authorize smoke fixture implementation yet",
    ])
...
def test_doc_declares_implementation_readiness_definition() -> None:
    assert_all_present(read(DOC), [
        "document-only pre-implementation contract",
        "readiness for a future controlled implementation",
        "uses only existing controlled synthetic JSON fixture",
        "uses only existing expected visible text fixture",
        "future output permitted only as text/plain or markdown/text compatible if a later phase authorizes it",
...
def test_doc_declares_implementation_readiness_criteria() -> None:
    assert_all_present(read(DOC), [
        "future controlled implementation readiness only",
...
def test_doc_declares_future_implementation_constraints() -> None:
    assert_all_present(read(DOC), [
