...
PHASE = "CID.LOCAL_MEDIA_AGENT.FFPROBE.CONTROLLED.FILE.METADATA.PREFLIGHT.VISIBLE.REPORT.RENDERER.CLI.VISIBLE.REPORT.OUTPUT.EXPORT.CONTROLLED.TEXT.ARTIFACT.CONTROLLED.EXPORT.PATH.IMPLEMENTATION.READINESS.CONTRACT.QA.GATE.V1"
PREVIOUS_PHASE = "CID.LOCAL_MEDIA_AGENT.FFPROBE.CONTROLLED.FILE.METADATA.PREFLIGHT.VISIBLE.REPORT.RENDERER.CLI.VISIBLE.REPORT.OUTPUT.EXPORT.CONTROLLED.TEXT.ARTIFACT.CONTROLLED.EXPORT.PATH.IMPLEMENTATION.READINESS.CONTRACT.V1"
PREVIOUS_RESULT = "LOCAL_MEDIA_AGENT_FFPROBE_CONTROLLED_FILE_METADATA_VISIBLE_REPORT_RENDERER_CLI_VISIBLE_REPORT_OUTPUT_EXPORT_CONTROLLED_TEXT_ARTIFACT_CONTROLLED_EXPORT_PATH_IMPLEMENTATION_READINESS_CONTRACT_PASS_READY_FOR_QA_GATE"
RESULT = "LOCAL_MEDIA_AGENT_FFPROBE_CONTROLLED_FILE_METADATA_VISIBLE_REPORT_RENDERER_CLI_VISIBLE_REPORT_OUTPUT_EXPORT_CONTROLLED_TEXT_ARTIFACT_CONTROLLED_EXPORT_PATH_IMPLEMENTATION_READINESS_CONTRACT_QA_GATE_PASS_CLOSED"
NEXT_PHASE = "CID.LOCAL_MEDIA_AGENT.FFPROBE.CONTROLLED.FILE.METADATA.PREFLIGHT.VISIBLE.REPORT.RENDERER.CLI.VISIBLE.REPORT.OUTPUT.EXPORT.CONTROLLED.TEXT.ARTIFACT.CONTROLLED.EXPORT.PATH.IMPLEMENTATION.READINESS.CONTRACT.QA.GATE.CLOSURE.REVIEW.V1"
...
def test_controlled_export_path_implementation_readiness_contract_qa_gate_files_exist():
...
def test_controlled_export_path_implementation_readiness_contract_qa_gate_declares_phase_result_and_next_phase():
    text = _read(DOC_PATH)
...
def test_controlled_export_path_implementation_readiness_contract_qa_gate_references_previous_closed_phase():
    text = _read(DOC_PATH)
...
def test_controlled_export_path_implementation_readiness_contract_qa_gate_is_doc_and_test_only():
    text = _read(DOC_PATH)
...
def test_controlled_export_path_implementation_readiness_contract_qa_gate_lists_assertions():
    text = _read(DOC_PATH)
...
def test_controlled_export_path_implementation_readiness_contract_qa_gate_lists_required_validation_evidence():
    text = _read(DOC_PATH)
...
def test_controlled_export_path_implementation_readiness_contract_qa_gate_closure_decision_is_limited():
    text = _read(DOC_PATH)

    assert "The controlled export path implementation readiness contract is valid and ready to be closed by this QA gate." in text
    assert "The chain may proceed only to a closure review for this QA gate." in text
    assert "This QA gate does not authorize implementation." in text
    assert "This QA gate does not authorize path resolver implementation, file writing, or artifact generation on disk." in text


def test_controlled_export_path_implementation_readiness_contract_qa_gate_non_authorization_is_complete():
    text = _read(DOC_PATH)
    lowered = text.lower()
...
def test_controlled_export_path_implementation_readiness_contract_qa_gate_previous_readiness_contract_contains_expected_result():
...
def test_controlled_export_path_implementation_readiness_contract_qa_gate_previous_readiness_contract_defines_scope():
    text = _read(READINESS_DOC_PATH)
...
def test_controlled_export_path_implementation_readiness_contract_qa_gate_previous_readiness_contract_defines_inputs_and_rejections():
    text = _read(READINESS_DOC_PATH)

    required_phrases = [
        "A controlled export root explicitly selected by a future controlled phase.",
        "A descriptor produced by the current controlled in-memory descriptor builder.",
        "The descriptor `suggested_filename` as the only filename candidate.",
        "Real media paths.",
        "Arbitrary folders.",
        "Scanner output from real material.",
        "User-supplied arbitrary output paths.",
        "Raw command-line path strings outside the controlled export root contract.",
        "Network locations.",
        "SaaS or database identifiers.",
    ]
...
def test_controlled_export_path_implementation_readiness_contract_qa_gate_previous_readiness_contract_defines_validation_rules():
    text = _read(READINESS_DOC_PATH)

    required_phrases = [
        "Empty controlled export roots.",
        "Empty suggested filenames.",
        "Whitespace-only suggested filenames.",
        "Absolute suggested filenames.",
        "Parent traversal segments.",
        "Path separators inside suggested filenames.",
        "Windows drive prefixes.",
        "UNC paths.",
        "Hidden dotfile names.",
        "Filenames without the `.controlled_visible_report.txt` suffix.",
        "Final resolved paths outside the controlled export root.",
    ]
...
def test_controlled_export_path_implementation_readiness_contract_qa_gate_previous_readiness_contract_defines_planned_descriptor():
    text = _read(READINESS_DOC_PATH)
...
def test_controlled_export_path_implementation_readiness_contract_qa_gate_previous_readiness_contract_boundary_is_explicit():
    text = _read(READINESS_DOC_PATH)
...
def test_controlled_export_path_implementation_readiness_contract_qa_gate_current_implementation_remains_pathless():
...
def test_controlled_export_path_implementation_readiness_contract_qa_gate_test_sources_have_no_external_execution_or_network_imports():
    for path in [TEST_PATH, READINESS_TEST_PATH, PREVIOUS_CLOSURE_TEST_PATH, PATH_CONTRACT_TEST_PATH]:
...
def test_controlled_export_path_implementation_readiness_contract_qa_gate_does_not_claim_runtime_or_client_readiness():
    combined = "\n".join(
        [
            _read(DOC_PATH),
...
def test_controlled_export_path_implementation_readiness_contract_qa_gate_result_is_closure_only():
    text = _read(DOC_PATH)

    assert RESULT in text
    assert "PATH_IMPLEMENTATION_READINESS_CONTRACT_QA_GATE_PASS_CLOSED" in RESULT
    assert NEXT_PHASE in text
