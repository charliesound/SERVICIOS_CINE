...
DOC = ROOT / "docs/product/local_media_agent/cid_local_media_agent_real_media_preflight_controlled_real_file_selection_readiness_gate_v1.md"
...
def test_controlled_real_file_selection_readiness_gate_doc_exists():
    assert DOC.exists()
...
def test_starting_state_is_from_manual_confirmation_gate():
    text = _text()
    assert "MANUAL_OPERATOR_CONFIRMATION_READY_FOR_CONTROLLED_REAL_FILE_SELECTION_READINESS_GATE" in text


def test_target_state_is_ready_for_controlled_real_file_selection_gate():
    text = _text()
...
def test_gate_purpose_is_readiness_only():
    text = _text()
    assert "This readiness gate prepares the conditions for a later controlled real file selection gate." in text
...
def test_source_manual_operator_confirmation_gate_is_referenced():
    text = _text()
...
def test_source_manual_confirmation_record_values_are_preserved():
    text = _text()
    required_values = [
...
def test_controlled_real_file_selection_readiness_record_is_declared():
    text = _text()
    required_fields = [
        "CONTROLLED_REAL_FILE_SELECTION_READINESS_RECORD_ID",
        "CONTROLLED_REAL_FILE_SELECTION_INPUT_RECORD_ID",
        "CONTROLLED_REAL_FILE_SELECTION_SOURCE_CONFIRMATION_RECORD_ID",
        "CONTROLLED_REAL_FILE_SELECTION_SOURCE_CONFIRMATION_HANDLE",
        "CONTROLLED_REAL_FILE_SELECTION_SOURCE_CONFIRMATION_VALUE",
        "CONTROLLED_REAL_FILE_SELECTION_SOURCE_TOKEN_RECORD_ID",
        "CONTROLLED_REAL_FILE_SELECTION_SOURCE_TOKEN_HANDLE",
        "CONTROLLED_REAL_FILE_SELECTION_SANITIZED_SELECTION_TOKEN",
        "CONTROLLED_REAL_FILE_SELECTION_SOURCE_REFERENCE_RECORD_ID",
        "CONTROLLED_REAL_FILE_SELECTION_SOURCE_REFERENCE_HANDLE",
        "CONTROLLED_REAL_FILE_SELECTION_SOURCE_EVENT_ID",
        "CONTROLLED_REAL_FILE_SELECTION_SOURCE_EVENT_HANDLE",
        "CONTROLLED_REAL_FILE_SELECTION_VERDICT",
    ]
    for field in required_fields:
        assert field in text


def test_controlled_real_file_selection_readiness_values_are_sanitized():
    text = _text()
    required_values = [
...
def test_readiness_status_is_ready_for_controlled_real_file_selection_gate():
    text = _text()
    assert "CONTROLLED_REAL_FILE_SELECTION_READINESS_STATUS" in text
    assert "ready_for_controlled_real_file_selection_gate" in text
    assert "The readiness status is `ready_for_controlled_real_file_selection_gate`." in text


def test_no_real_file_selection_occurs_in_this_gate():
    text = _text()
    assert "CONTROLLED_REAL_FILE_SELECTION_REAL_SELECTION_STATUS" in text
...
def test_no_real_path_is_resolved_or_recorded():
    text = _text()
    assert "CONTROLLED_REAL_FILE_SELECTION_REAL_PATH_STATUS" in text
...
def test_no_filename_or_parent_folder_is_recorded():
    text = _text()
    required_statuses = [
        "CONTROLLED_REAL_FILE_SELECTION_REAL_FILENAME_STATUS",
        "CONTROLLED_REAL_FILE_SELECTION_PARENT_FOLDER_STATUS",
        "not_recorded_in_this_gate",
    ]
    for status in required_statuses:
        assert status in text
    assert "No real filename is recorded in this gate." in text
...
def test_no_file_size_timestamps_or_hashes_are_recorded():
    text = _text()
    required_statuses = [
        "CONTROLLED_REAL_FILE_SELECTION_FILE_SIZE_STATUS",
        "CONTROLLED_REAL_FILE_SELECTION_TIMESTAMP_STATUS",
        "CONTROLLED_REAL_FILE_SELECTION_HASH_STATUS",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_filesystem_or_file_open_operations_occur():
    text = _text()
    required_statuses = [
        "CONTROLLED_REAL_FILE_SELECTION_FILESYSTEM_METADATA_STATUS",
        "not_read_in_this_gate",
        "CONTROLLED_REAL_FILE_SELECTION_FILE_OPEN_STATUS",
...
def test_no_execution_or_media_tools_are_run():
    text = _text()
    required_statuses = [
        "CONTROLLED_REAL_FILE_SELECTION_EXECUTION_STATUS",
        "CONTROLLED_REAL_FILE_SELECTION_FFMPEG_STATUS",
        "CONTROLLED_REAL_FILE_SELECTION_FFPROBE_STATUS",
        "CONTROLLED_REAL_FILE_SELECTION_SCANNER_STATUS",
...
def test_no_runtime_or_saas_integration_is_created():
    text = _text()
    assert "CONTROLLED_REAL_FILE_SELECTION_RUNTIME_STATUS" in text
    assert "no_runtime_created" in text
    assert "CONTROLLED_REAL_FILE_SELECTION_SAAS_STATUS" in text
...
def test_positive_assertions_preserve_scope():
    ...
    required_assertions = [
        ...
        "`controlled_real_file_selection_readiness_001` is created as a readiness record.",
...
def test_later_controlled_real_file_selection_constraints_are_conservative():
    text = _text()
    constraints = [
...
def test_next_phase_boundary_is_controlled_real_file_selection_gate_only():
    text = _text()
    assert "The next conservative phase may define a controlled real file selection gate." in text
    ...
    assert "This readiness gate only prepares the conditions for a later controlled real file selection gate." in text


def test_required_checks_reference_previous_gates_and_generic_guards():
    text = _text()
    required_checks = [
        "This controlled real file selection readiness gate test.",
...
def test_closing_state_is_ready_for_controlled_real_file_selection_gate():
    text = _text()
    assert "READY_FOR_CONTROLLED_REAL_FILE_SELECTION_GATE" in text
