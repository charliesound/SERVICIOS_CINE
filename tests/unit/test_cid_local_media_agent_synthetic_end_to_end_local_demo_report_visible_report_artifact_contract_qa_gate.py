...
QA_DOC = Path("docs/product/local_media_agent/cid_local_media_agent_synthetic_end_to_end_local_demo_report_visible_report_artifact_contract_qa_gate_v1.md")
ARTIFACT_DOC = Path("docs/product/local_media_agent/cid_local_media_agent_synthetic_end_to_end_local_demo_report_visible_report_artifact_contract_v1.md")
ARTIFACT_TEST = Path("tests/unit/test_cid_local_media_agent_synthetic_end_to_end_local_demo_report_visible_report_artifact_contract.py")
MAPPING_QA = Path("docs/product/local_media_agent/cid_local_media_agent_synthetic_end_to_end_local_demo_report_visible_report_template_fixture_mapping_contract_qa_gate_v1.md")
...
TEMPLATE_CONTRACT = Path("docs/product/local_media_agent/cid_local_media_agent_synthetic_end_to_end_local_demo_report_visible_report_template_contract_v1.md")
...
def test_phase_upstream_commit_and_tag():
    ...
    assert "d3d3074" in text
    assert "visible-report-artifact-contract-v1-20260618" in text

def test_required_upstream_files_exist_and_are_referenced():
    text = read(QA_DOC)
    for path in [ARTIFACT_DOC, ARTIFACT_TEST, MAPPING_QA, MAPPING_CONTRACT, TEMPLATE_CONTRACT, FIXTURE]:
        assert path.exists()
...
def test_opencode_must_not_write_or_implement():
    text = read(QA_DOC)
    for term in ["edit files", "stage files", "commit", "tag", "push", "create artifacts", "create renderer code", "create generator code", "create loader code", "create runtime code"]:
        assert term in text

def test_blocked_artifacts_runtime_media_and_exports():
    text = read(QA_DOC)
    for term in ["create HTML", "create PDF", "create DOCX", "create XLSX", "create Markdown report artifact", "execute ffprobe", "execute ffmpeg", "inspect real media", "process media", "create subtitles", "create NLE exports", "touch backend", "touch frontend", "touch database", "touch Alembic", "touch Docker"]:
        assert term in text

def test_audit_brief_preserves_product_safety():
    text = read(QA_DOC)
    for term in ["local-first privacy", "Spanish-first stakeholder readability", "required visible disclaimers", "no false claims", "mandatory human review", "assistive and not substitutive"]:
        assert term in text
