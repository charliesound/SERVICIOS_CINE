...
DOC = ROOT / "docs/product/local_media_agent/cid_local_media_agent_real_media_preflight_stat_execution_readiness_gate_v1.md"
...
def test_stat_execution_readiness_gate_doc_exists():
    assert DOC.exists()
...
def test_starting_state_is_from_controlled_stat_gate():
    text = _text()
    assert "CONTROLLED_STAT_BOUNDARY_READY_FOR_STAT_EXECUTION_READINESS_GATE" in text


def test_target_state_is_ready_for_stat_execution_gate():
    text = _text()
...
def test_gate_purpose_is_readiness_only():
    text = _text()
    assert "This readiness gate prepares the conditions for a later stat execution gate." in text
...
def test_source_controlled_stat_gate_is_referenced():
    text = _text()
...
def test_source_controlled_stat_boundary_record_values_are_preserved():
    text = _text()
    required_values = [
...
def test_stat_execution_readiness_record_is_declared():
    text = _text()
    required_fields = [
        "STAT_EXECUTION_READINESS_RECORD_ID",
        "STAT_EXECUTION_INPUT_RECORD_ID",
        "STAT_EXECUTION_SOURCE_CONTROLLED_STAT_BOUNDARY_RECORD_ID",
        "STAT_EXECUTION_SOURCE_CONTROLLED_STAT_BOUNDARY_HANDLE",
        "STAT_EXECUTION_SOURCE_REAL_FILE_ACCESS_BOUNDARY_RECORD_ID",
        "STAT_EXECUTION_SOURCE_REAL_FILE_ACCESS_BOUNDARY_HANDLE",
        "STAT_EXECUTION_SOURCE_LOCAL_PATH_DISCLOSURE_BOUNDARY_RECORD_ID",
        "STAT_EXECUTION_SOURCE_LOCAL_PATH_DISCLOSURE_BOUNDARY_HANDLE",
        "STAT_EXECUTION_SOURCE_SELECTION_BOUNDARY_RECORD_ID",
        "STAT_EXECUTION_SOURCE_SELECTION_BOUNDARY_HANDLE",
        "STAT_EXECUTION_SOURCE_CONFIRMATION_RECORD_ID",
        "STAT_EXECUTION_SOURCE_CONFIRMATION_HANDLE",
        "STAT_EXECUTION_SOURCE_CONFIRMATION_VALUE",
        "STAT_EXECUTION_SOURCE_TOKEN_RECORD_ID",
        "STAT_EXECUTION_SOURCE_TOKEN_HANDLE",
        "STAT_EXECUTION_SANITIZED_SELECTION_TOKEN",
        "STAT_EXECUTION_SOURCE_REFERENCE_RECORD_ID",
        "STAT_EXECUTION_SOURCE_REFERENCE_HANDLE",
        "STAT_EXECUTION_SOURCE_EVENT_ID",
        "STAT_EXECUTION_SOURCE_EVENT_HANDLE",
        "STAT_EXECUTION_VERDICT",
    ]
    for field in required_fields:
        assert field in text


def test_stat_execution_readiness_values_are_sanitized():
    text = _text()
    required_values = [
...
def test_readiness_status_is_ready_for_stat_execution_gate():
    text = _text()
    assert "STAT_EXECUTION_READINESS_STATUS" in text
    assert "ready_for_stat_execution_gate" in text
    assert "The readiness status is `ready_for_stat_execution_gate`." in text


def test_no_stat_execution_occurs_in_this_gate():
    text = _text()
    assert "STAT_EXECUTION_STAT_STATUS" in text
...
def test_no_real_file_access_occurs_in_this_gate():
    text = _text()
    assert "STAT_EXECUTION_ACCESS_STATUS" in text
...
def test_no_media_file_is_opened_in_this_gate():
    text = _text()
    assert "STAT_EXECUTION_FILE_OPEN_STATUS" in text
...
def test_no_file_bytes_or_real_metadata_are_read():
    text = _text()
    required_statuses = [
        "STAT_EXECUTION_FILE_BYTES_STATUS",
        "STAT_EXECUTION_FILESYSTEM_METADATA_STATUS",
...
def test_no_file_size_timestamps_or_hashes_are_recorded():
    text = _text()
    required_statuses = [
        "STAT_EXECUTION_FILE_SIZE_STATUS",
        "STAT_EXECUTION_TIMESTAMP_STATUS",
        "STAT_EXECUTION_HASH_STATUS",
...
def test_no_local_path_or_identity_metadata_is_committed():
    text = _text()
    required_statuses = [
        "STAT_EXECUTION_REAL_PATH_STATUS",
        "STAT_EXECUTION_ABSOLUTE_PATH_STATUS",
        "STAT_EXECUTION_RELATIVE_PATH_STATUS",
        "not_recorded_in_committed_artifacts",
        "STAT_EXECUTION_REAL_FILENAME_STATUS",
        "STAT_EXECUTION_PARENT_FOLDER_STATUS",
        "not_recorded_in_this_gate",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_media_decode_probe_scan_or_transcription_occurs():
    text = _text()
    required_statuses = [
        "STAT_EXECUTION_MEDIA_DECODE_STATUS",
        "STAT_EXECUTION_MEDIA_PROBE_STATUS",
        "STAT_EXECUTION_MEDIA_SCAN_STATUS",
        "STAT_EXECUTION_TRANSCRIPTION_STATUS",
...
def test_no_thumbnails_or_waveforms_are_generated():
    text = _text()
    required_statuses = [
        "STAT_EXECUTION_THUMBNAIL_STATUS",
        "not_generated_in_this_gate",
        "STAT_EXECUTION_WAVEFORM_STATUS",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_execution_or_media_tools_are_run():
    text = _text()
    required_statuses = [
        "STAT_EXECUTION_EXECUTION_STATUS",
        "STAT_EXECUTION_FFMPEG_STATUS",
        "STAT_EXECUTION_FFPROBE_STATUS",
        "STAT_EXECUTION_SCANNER_STATUS",
        "not_executed_in_this_gate",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_runtime_or_saas_integration_is_created():
    text = _text()
    assert "STAT_EXECUTION_RUNTIME_STATUS" in text
    assert "no_runtime_created" in text
    assert "STAT_EXECUTION_SAAS_STATUS" in text
...
def test_positive_assertions_preserve_scope():
    ...
    required_assertions = [
        ...
        "`stat_execution_readiness_001` is created as a readiness record.",
...
def test_later_stat_execution_constraints_are_conservative():
    text = _text()
    constraints = [
...
def test_next_phase_boundary_is_stat_execution_gate_only():
    text = _text()
    assert "The next conservative phase may define a stat execution gate." in text
    assert "This stat execution readiness gate does not authorize filesystem stat execution." in text
    assert "This stat execution readiness gate does not authorize accessing a real file." in text
    assert "This stat execution readiness gate does not authorize opening media." in text
    assert "This stat execution readiness gate does not authorize reading file bytes." in text
    assert "This stat execution readiness gate does not authorize reading real metadata." in text
    assert "This stat execution readiness gate does not authorize media execution." in text
    assert "This stat execution readiness gate only prepares conditions for a later stat execution gate." in text


def test_required_checks_reference_previous_gates_and_generic_guards():
    text = _text()
    required_checks = [
        "This stat execution readiness gate test.",
...
def test_closing_state_is_ready_for_stat_execution_gate():
    text = _text()
    assert "READY_FOR_STAT_EXECUTION_GATE" in text
