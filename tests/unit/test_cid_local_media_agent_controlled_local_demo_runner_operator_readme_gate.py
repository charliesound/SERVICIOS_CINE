...
DOC_PATH = REPO_ROOT / "docs/product/local_media_agent/cid_local_media_agent_controlled_local_demo_runner_operator_readme_gate_v1.md"
...
PHASE = (
    ...
    "CONTROLLED.LOCAL.DEMO.RUNNER.OPERATOR.README.GATE.V1"
)
...
EXPECTED_RESULT = "LOCAL_MEDIA_AGENT_CONTROLLED_LOCAL_DEMO_RUNNER_OPERATOR_README_GATE_V1_CLOSED"
...
def _assert_operator_safe_payload(payload: dict) -> None:
...
def test_operator_readme_freezes_internal_runbook() -> None:
    ...
    assert "The operator must remove the reported output_root after inspection." in text
    assert "rm -rf" in text
    assert "cid-lma-controlled-demo-*" in text


def test_operator_help_command_matches_readme_surface() -> None:
...
def test_operator_default_json_run_is_self_cleaning() -> None:
    payload = _load_runner_json()

    _assert_operator_safe_payload(payload)
...
def test_operator_keep_output_run_is_inspectable_and_cleaned_by_test_owner() -> None:
