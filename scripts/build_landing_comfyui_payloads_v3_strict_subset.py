...
PROMPTS_PATH = ROOT / "src/comfyui_workflows/landing_semantic_v3/landing_semantic_prompts_v3_strict.json"
...
PAYLOADS_DIR = ROOT / ".tmp" / "landing_comfyui_v3" / "strict_image_payloads"
...
def deterministic_seed(image_key: str) -> int:
    digest = hashlib.sha256((image_key + "-strict").encode("utf-8")).hexdigest()
...
def main() -> int:
    ...
    for image_key in subset_keys:
        ...
        if isinstance(save_image_node, dict):
            save_image_node.setdefault("inputs", {})["filename_prefix"] = f"landing_v3_strict/{image_key}"
        ...
        payload = {
            ...
            "render_mode": "strict_subset_dry_run",
        }
        validate_payload_text(payload)
        dump_json(PAYLOADS_DIR / f"{image_key}.json", payload)

    print(f"Strict subset payloads generated in {PAYLOADS_DIR.relative_to(ROOT)}")
