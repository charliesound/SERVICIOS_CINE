...
DOC = ROOT / "docs/product/local_media_agent/cid_local_media_agent_real_media_preflight_code_skeleton_gate_v1.md"
...
def test_code_skeleton_gate_doc_exists():
    assert DOC.exists()


def test_code_skeleton_module_exists():
...
def test_starting_state_is_ready_for_code_skeleton_gate():
    text = _doc_text()
    assert "READY_FOR_CODE_SKELETON_GATE" in text


def test_target_state_is_ready_for_isolated_contract_qa_gate():
    text = _doc_text()
    assert "CODE_SKELETON_CREATED_READY_FOR_ISOLATED_CONTRACT_QA_GATE" in text


def test_gate_purpose_creates_non_executing_skeleton_only():
    text = _doc_text()
    assert "This gate creates the first isolated code skeleton for controlled stat preflight." in text
    assert "This gate creates a non-executing Python skeleton module." in text
...
def test_created_artifacts_are_declared():
    text = _doc_text()
    assert "real_media_preflight_controlled_stat_code_skeleton.py" in text
    assert "cid_local_media_agent_real_media_preflight_code_skeleton_gate_v1.py" in text


def test_code_skeleton_record_is_declared():
    text = _doc_text()
    required_fields = [
        "CODE_SKELETON_RECORD_ID",
        "CODE_SKELETON_INPUT_RECORD_ID",
        "CODE_SKELETON_SOURCE_READINESS_RECORD_ID",
...
def test_code_skeleton_record_values_are_sanitized():
    text = _doc_text()
    required_values = [
        "code_skeleton_001",
        "CODE_SKELETON_HANDLE_001",
        "created_as_non_executing_isolated_skeleton",
        "isolated_controlled_stat_skeleton_only",
        "skeleton_created_without_real_stat_execution",
...
def test_skeleton_behavior_is_documented():
    text = _doc_text()
    requirements = [
        "A sanitized input dataclass.",
        "A sanitized output dataclass.",
        "A pure planning helper.",
        "A pure redaction helper.",
        "A boundary constants object.",
...
def test_skeleton_module_defines_expected_public_shapes():
    text = _skeleton_text()
    required_symbols = [
        "ControlledStatSkeletonInput",
...
def test_skeleton_plan_is_pure_and_non_executing():
    ...
    assert plan.boundary_handle == BOUNDARY_HANDLE
...
def test_redaction_replaces_local_token_with_fixed_sanitized_token():
    control_input = ControlledStatSkeletonInput(
        input_record_id="operator_input_001",
        sanitized_selection_token="LOCAL_TEST_TOKEN",
        manual_confirmation_handle="MANUAL_OPERATOR_CONFIRMATION_HANDLE_001",
    ...
    assert redacted["sanitized_selection_token"] == SANITIZED_SELECTION_TOKEN
    assert "LOCAL_TEST_TOKEN" not in redacted.values()


def test_safety_boundary_reports_only_non_execution_statuses():
...
def test_positive_assertions_preserve_source_scope():
    text = _doc_text()
    required_assertions = [
        "`code_skeleton_001` is created as a sanitized skeleton record.",
        "`CODE_SKELETON_HANDLE_001` is a non-filesystem skeleton handle.",
        "`code_skeleton_readiness_001` remains the source readiness record.",
...
def test_next_phase_boundary_is_isolated_contract_qa_gate():
    text = _doc_text()
    assert "The next conservative phase may be an isolated contract QA gate for the skeleton." in text
    assert "This code skeleton gate does not authorize filesystem stat execution." in text
    assert "This code skeleton gate does not authorize accessing a real file." in text
    assert "This code skeleton gate does not authorize opening media." in text
    assert "This code skeleton gate does not authorize reading file bytes." in text
    assert "This code skeleton gate does not authorize reading real metadata." in text
    assert "This code skeleton gate does not authorize media execution." in text
    assert "This code skeleton gate only creates a non-executing isolated code skeleton." in text


def test_required_checks_reference_previous_gates_and_generic_guards():
    text = _doc_text()
    required_checks = [
        "This code skeleton gate test.",
...
def test_doc_does_not_contain_runtime_invocation_patterns():
    text = _doc_text()
    forbidden_runtime_patterns = [
        "subprocess.run",
        "subprocess.Popen",
...
def test_closing_state_is_ready_for_isolated_contract_qa_gate():
    text = _doc_text()
    assert "CODE_SKELETON_CREATED_READY_FOR_ISOLATED_CONTRACT_QA_GATE" in text
