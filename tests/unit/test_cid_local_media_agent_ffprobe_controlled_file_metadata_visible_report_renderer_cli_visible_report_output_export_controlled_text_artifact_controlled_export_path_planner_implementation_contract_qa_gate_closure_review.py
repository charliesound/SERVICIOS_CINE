...
PHASE = "CID.LOCAL_MEDIA_AGENT.FFPROBE.CONTROLLED.FILE.METADATA.PREFLIGHT.VISIBLE.REPORT.RENDERER.CLI.VISIBLE.REPORT.OUTPUT.EXPORT.CONTROLLED.TEXT.ARTIFACT.CONTROLLED.EXPORT.PATH.PLANNER.IMPLEMENTATION.CONTRACT.QA.GATE.CLOSURE.REVIEW.V1"
PREVIOUS_PHASE = "CID.LOCAL_MEDIA_AGENT.FFPROBE.CONTROLLED.FILE.METADATA.PREFLIGHT.VISIBLE.REPORT.RENDERER.CLI.VISIBLE.REPORT.OUTPUT.EXPORT.CONTROLLED.TEXT.ARTIFACT.CONTROLLED.EXPORT.PATH.PLANNER.IMPLEMENTATION.CONTRACT.QA.GATE.V1"
PREVIOUS_RESULT = "LOCAL_MEDIA_AGENT_FFPROBE_CONTROLLED_FILE_METADATA_VISIBLE_REPORT_RENDERER_CLI_VISIBLE_REPORT_OUTPUT_EXPORT_CONTROLLED_TEXT_ARTIFACT_CONTROLLED_EXPORT_PATH_PLANNER_IMPLEMENTATION_CONTRACT_QA_GATE_PASS_CLOSED"
RESULT = "LOCAL_MEDIA_AGENT_FFPROBE_CONTROLLED_FILE_METADATA_VISIBLE_REPORT_RENDERER_CLI_VISIBLE_REPORT_OUTPUT_EXPORT_CONTROLLED_TEXT_ARTIFACT_CONTROLLED_EXPORT_PATH_PLANNER_IMPLEMENTATION_CONTRACT_QA_GATE_CLOSURE_REVIEW_PASS_READY_FOR_CONTROLLED_PATH_PLANNER_IMPLEMENTATION_READINESS_GATE"
NEXT_PHASE = "CID.LOCAL_MEDIA_AGENT.FFPROBE.CONTROLLED.FILE.METADATA.PREFLIGHT.VISIBLE.REPORT.RENDERER.CLI.VISIBLE.REPORT.OUTPUT.EXPORT.CONTROLLED.TEXT.ARTIFACT.CONTROLLED.EXPORT.PATH.PLANNER.IMPLEMENTATION.READINESS.GATE.V1"
...
def test_controlled_export_path_planner_contract_qa_gate_closure_review_files_exist():
...
def test_controlled_export_path_planner_contract_qa_gate_closure_review_declares_phase_result_and_next_phase():
    text = _read(DOC_PATH)
...
def test_controlled_export_path_planner_contract_qa_gate_closure_review_references_previous_closed_gate():
    text = _read(DOC_PATH)
...
def test_controlled_export_path_planner_contract_qa_gate_closure_review_is_doc_and_test_only():
    text = _read(DOC_PATH)
...
def test_controlled_export_path_planner_contract_qa_gate_closure_review_summarizes_gate_findings():
    text = _read(DOC_PATH)
...
def test_controlled_export_path_planner_contract_qa_gate_closure_review_decision_is_limited():
    text = _read(DOC_PATH)

    assert "The controlled export path planner implementation contract QA gate is closed." in text
...
def test_controlled_export_path_planner_contract_qa_gate_closure_review_non_authorization_is_complete():
    text = _read(DOC_PATH)
    lowered = text.lower()
...
def test_controlled_export_path_planner_contract_qa_gate_closure_review_previous_gate_contains_expected_result():
    text = _read(QA_GATE_DOC_PATH)

    assert PREVIOUS_PHASE in text
    assert PREVIOUS_RESULT in text
    assert "This QA gate validates the controlled export path planner implementation contract." in text
    assert "This phase is documentation and test-only." in text
    assert "This QA gate does not authorize implementation." in text
    assert "PATH_PLANNER_IMPLEMENTATION_CONTRACT_QA_GATE_PASS_CLOSED" in text


def test_controlled_export_path_planner_contract_qa_gate_closure_review_previous_gate_defines_validation_evidence():
    text = _read(QA_GATE_DOC_PATH)
...
def test_controlled_export_path_planner_contract_qa_gate_closure_review_planner_contract_contains_expected_result():
    ...
    assert "This contract only defines what a future controlled implementation phase may implement." in text


def test_controlled_export_path_planner_contract_qa_gate_closure_review_planner_contract_defines_core_scope():
    text = _read(PLANNER_CONTRACT_DOC_PATH)
...
def test_controlled_export_path_planner_contract_qa_gate_closure_review_planner_contract_defines_input_validation():
    text = _read(PLANNER_CONTRACT_DOC_PATH)
...
def test_controlled_export_path_planner_contract_qa_gate_closure_review_planner_contract_defines_descriptor_and_false_flags():
    text = _read(PLANNER_CONTRACT_DOC_PATH)
...
def test_controlled_export_path_planner_contract_qa_gate_closure_review_current_implementation_remains_pathless():
...
def test_controlled_export_path_planner_contract_qa_gate_closure_review_test_sources_have_no_external_execution_or_network_imports():
    for path in [
        TEST_PATH,
        QA_GATE_TEST_PATH,
        PLANNER_CONTRACT_TEST_PATH,
        READINESS_CLOSURE_TEST_PATH,
        READINESS_QA_GATE_TEST_PATH,
        READINESS_TEST_PATH,
...
def test_controlled_export_path_planner_contract_qa_gate_closure_review_does_not_claim_runtime_or_client_readiness():
    combined = "\n".join(
        [
            _read(DOC_PATH),
