"""
Contract test: shotlist_routes.py gating invariants.

Verifies that all shotlist endpoints require tenant context,
...
TARGET_FILE = ROUTES_DIR / "shotlist_routes.py"
...
def test_router_has_get_tenant_context_dependency():
    source = _read_source()
    match = re.search(
        r'router = APIRouter\(prefix="/api/projects/\{project_id\}/planned-shots".*dependencies=\[Depends\(get_tenant_context\)\]',
        source,
    )
    assert match, (
...
def test_all_endpoints_have_validate_project_access():
    ...
    assert count >= 5, (
        f"Expected >=5 occurrences of validate_project_access, found {count}"
    )
...
def test_write_endpoints_have_require_write_permission():
    ...
    assert count >= 3, (
        f"Expected >=3 occurrences of require_write_permission, found {count}"
    )
...
def test_approve_validates_shot_belongs_to_project():
    source = _read_source()
    idx = source.index('@router.post("/{shot_id}/approve")')
    block = source[idx:]
    next_dec = re.search(r"\n@", block)
    func = block[:next_dec.start()] if next_dec else block
    assert "select(PlannedShot)" in func, (
        "approve endpoint must query PlannedShot"
    )
    assert "shot.project_id != project_id" in func or "planned_shot.project_id != project_id" in func, (
        "approve endpoint must verify shot.project_id matches project_id"
    )
    assert "HTTPException(status_code=404" in func, (
        "approve endpoint must raise 404 if shot not found or mismatched"
    )


def test_reject_validates_shot_belongs_to_project():
    source = _read_source()
    idx = source.index('@router.post("/{shot_id}/reject")')
    block = source[idx:]
    next_dec = re.search(r"\n@", block)
    func = block[:next_dec.start()] if next_dec else block
    assert "select(PlannedShot)" in func, (
        "reject endpoint must query PlannedShot"
    )
    assert "shot.project_id != project_id" in func or "planned_shot.project_id != project_id" in func, (
        "reject endpoint must verify shot.project_id matches project_id"
    )
    assert "HTTPException(status_code=404" in func, (
        "reject endpoint must raise 404 if shot not found or mismatched"
    )
