...
TEST_DB_PATH = Path("/tmp/test_script_analysis_export.db")
TEST_STORAGE_ROOT = Path("/tmp/test_script_analysis_export_storage")
...
class ScriptAnalysisExportIntegrationTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.headers = _auth_headers(USER_ID, "export@example.com")
    ...
    def test_export_json_returns_200_with_structure(self) -> None:
        response = self.client.get(
            f"/api/projects/{PROJECT_ID}/analysis/export?format=json",
            headers=self.headers,
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.headers.get("content-type"), "application/json")
        disp = response.headers.get("content-disposition", "")
        self.assertIn("CID_script_analysis_", disp)
        self.assertIn(".json", disp)

        data = response.json()
        self.assertEqual(data["project_id"], PROJECT_ID)
        self.assertEqual(data["export_version"], "1.0")
        self.assertEqual(data["source"], "cid_script_analysis_pro")
        self.assertTrue(data["has_analysis"])
        self.assertTrue(data["has_script"])
        self.assertIn("logline", data)
        self.assertIn("synopsis_short", data)
    ...
    def test_export_json_includes_synopsis_fields(self) -> None:
        response = self.client.get(
            f"/api/projects/{PROJECT_ID}/analysis/export?format=json",
            headers=self.headers,
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertGreater(len(data.get("logline", "")), 0)
        self.assertGreater(len(data.get("synopsis_short", "")), 0)
        self.assertGreater(len(data.get("genre", "")), 0)
        self.assertGreater(len(data.get("tone", "")), 0)

    def test_export_json_includes_analysis_data(self) -> None:
        response = self.client.get(
            f"/api/projects/{PROJECT_ID}/analysis/export?format=json",
            headers=self.headers,
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertEqual(data["analysis_engine"], "heuristic")
        self.assertEqual(len(data["scenes"]), 2)
        self.assertEqual(len(data["characters"]), 2)
        self.assertEqual(len(data["locations"]), 2)
        self.assertEqual(len(data["breakdowns"]), 2)
        self.assertIn("departments", data["department_breakdown"])

    # --- Export Markdown ---

    def test_export_md_returns_200_with_content(self) -> None:
        response = self.client.get(
            f"/api/projects/{PROJECT_ID}/analysis/export?format=md",
            headers=self.headers,
        )
        self.assertEqual(response.status_code, 200)
        ...
        body = response.text
        self.assertIn("CID Script Analysis Pro", body)
    ...
    def test_export_invalid_format_returns_422(self) -> None:
        response = self.client.get(
            f"/api/projects/{PROJECT_ID}/analysis/export?format=pdf",
            headers=self.headers,
        )
    ...
    def test_export_nonexistent_project_returns_404(self) -> None:
        response = self.client.get(
            "/api/projects/nonexistent-project/analysis/export?format=json",
            headers=self.headers,
        )
    ...
    def test_export_json_no_analysis_still_returns_synopsis(self) -> None:
        NO_ANALYSIS_PROJECT = "project-export-no-analysis-0001"
        connection = sqlite3.connect(str(TEST_DB_PATH))
        try:
            _insert_row(
                connection,
                "projects",
                {
                    "id": NO_ANALYSIS_PROJECT,
                    "organization_id": ORG_ID,
                    "name": "No Analysis Project",
                    "description": "Only has script text",
                    "status": "development",
                    "script_text": "1 EXT. CALLE. DIA.\n\nAccion.",
                },
            )
            connection.commit()
        finally:
        ...
        response = self.client.get(
            f"/api/projects/{NO_ANALYSIS_PROJECT}/analysis/export?format=json",
            headers=self.headers,
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
        ...
        self.assertIn("no_analysis_found", data["warnings"])
