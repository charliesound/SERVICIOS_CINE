...
DOC = ROOT / "docs/product/local_media_agent/cid_local_media_agent_real_media_preflight_local_path_disclosure_gate_v1.md"
...
def test_local_path_disclosure_gate_doc_exists():
    assert DOC.exists()
...
def test_starting_state_is_ready_for_local_path_disclosure_gate():
    text = _text()
    assert "READY_FOR_LOCAL_PATH_DISCLOSURE_GATE" in text


def test_target_state_prepares_real_file_access_readiness():
    text = _text()
...
def test_gate_purpose_defines_boundary_only():
    text = _text()
    assert "This gate defines the local path disclosure boundary for a later operator-local real file access readiness phase." in text
    assert "This gate creates only a sanitized local path disclosure boundary record." in text
    assert "This gate does not disclose a local filesystem path in committed artifacts." in text
    assert "This gate does not record a real path in committed artifacts." in text
    assert "This gate is limited to documentation and tests." in text


def test_source_local_path_disclosure_readiness_gate_is_referenced():
    text = _text()
...
def test_source_local_path_disclosure_readiness_record_values_are_preserved():
    text = _text()
    required_values = [
...
def test_local_path_disclosure_boundary_record_is_declared():
    text = _text()
    required_fields = [
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_RECORD_ID",
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_INPUT_RECORD_ID",
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_SOURCE_READINESS_RECORD_ID",
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_SOURCE_SELECTION_BOUNDARY_RECORD_ID",
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_SOURCE_SELECTION_BOUNDARY_HANDLE",
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_SOURCE_CONFIRMATION_RECORD_ID",
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_SOURCE_CONFIRMATION_HANDLE",
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_SOURCE_CONFIRMATION_VALUE",
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_SOURCE_TOKEN_RECORD_ID",
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_SOURCE_TOKEN_HANDLE",
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_SANITIZED_SELECTION_TOKEN",
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_SOURCE_REFERENCE_RECORD_ID",
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_SOURCE_REFERENCE_HANDLE",
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_SOURCE_EVENT_ID",
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_SOURCE_EVENT_HANDLE",
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_HANDLE",
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_VERDICT",
    ]
    for field in required_fields:
        assert field in text


def test_local_path_disclosure_boundary_values_are_sanitized():
    text = _text()
    required_values = [
        "local_path_disclosure_boundary_001",
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_HANDLE_001",
        "defined_as_operator_local_disclosure_boundary",
        "prepares_real_file_access_readiness_only",
        "boundary_defined_without_committed_path_value",
...
def test_boundary_status_and_scope_are_limited():
    text = _text()
    assert "LOCAL_PATH_DISCLOSURE_BOUNDARY_STATUS" in text
    assert "defined_as_operator_local_disclosure_boundary" in text
    assert "LOCAL_PATH_DISCLOSURE_BOUNDARY_SCOPE_STATUS" in text
    assert "prepares_real_file_access_readiness_only" in text
    assert "The boundary status is `defined_as_operator_local_disclosure_boundary`." in text
    assert "The boundary scope prepares real file access readiness only." in text


def test_boundary_does_not_commit_local_path_value():
    text = _text()
    assert "LOCAL_PATH_DISCLOSURE_BOUNDARY_DISCLOSURE_STATUS" in text
    assert "boundary_defined_without_committed_path_value" in text
    assert "The boundary does not commit a local path value." in text


def test_no_real_file_selection_occurs():
    text = _text()
    assert "LOCAL_PATH_DISCLOSURE_BOUNDARY_REAL_SELECTION_STATUS" in text
    assert "not_selected" in text
    assert "No real file is selected." in text


def test_no_real_absolute_or_relative_path_is_recorded_in_committed_artifacts():
    text = _text()
    required_statuses = [
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_REAL_PATH_STATUS",
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_ABSOLUTE_PATH_STATUS",
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_RELATIVE_PATH_STATUS",
        "not_recorded_in_committed_artifacts",
    ]
    for status in required_statuses:
        assert status in text
    assert "No real path is recorded in committed artifacts." in text
    assert "No absolute path is recorded in committed artifacts." in text
    assert "No relative path is recorded in committed artifacts." in text


def test_no_filename_or_parent_folder_is_recorded():
    text = _text()
    assert "LOCAL_PATH_DISCLOSURE_BOUNDARY_REAL_FILENAME_STATUS" in text
    assert "LOCAL_PATH_DISCLOSURE_BOUNDARY_PARENT_FOLDER_STATUS" in text
...
def test_no_file_size_timestamps_or_hashes_are_recorded():
    text = _text()
    required_statuses = [
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_FILE_SIZE_STATUS",
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_TIMESTAMP_STATUS",
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_HASH_STATUS",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_filesystem_or_file_open_operations_occur():
    text = _text()
    required_statuses = [
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_FILESYSTEM_METADATA_STATUS",
        "not_read",
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_FILE_OPEN_STATUS",
        "not_opened",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_execution_or_media_tools_are_run():
    text = _text()
    required_statuses = [
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_EXECUTION_STATUS",
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_FFMPEG_STATUS",
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_FFPROBE_STATUS",
        "LOCAL_PATH_DISCLOSURE_BOUNDARY_SCANNER_STATUS",
        "not_executed",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_runtime_or_saas_integration_is_created():
    text = _text()
    assert "LOCAL_PATH_DISCLOSURE_BOUNDARY_RUNTIME_STATUS" in text
    assert "no_runtime_created" in text
    assert "LOCAL_PATH_DISCLOSURE_BOUNDARY_SAAS_STATUS" in text
...
def test_positive_assertions_preserve_scope():
    ...
    required_assertions = [
        ...
        "`local_path_disclosure_boundary_001` is created as a sanitized boundary record.",
        "`LOCAL_PATH_DISCLOSURE_BOUNDARY_HANDLE_001` is a non-filesystem boundary handle.",
        "`local_path_disclosure_readiness_001` remains the source readiness record.",
...
def test_real_file_access_readiness_constraints_are_conservative():
    text = _text()
    constraints = [
...
def test_committed_path_disclosure_is_forbidden():
    text = _text()
    forbidden_items = [
...
def test_next_phase_boundary_is_real_file_access_readiness_only():
    text = _text()
    assert "The next conservative phase may prepare a real file access readiness gate." in text
    assert "This local path disclosure gate does not authorize committing local paths." in text
    assert "This local path disclosure gate does not authorize real file access." in text
    assert "This local path disclosure gate does not authorize filesystem metadata reading." in text
    assert "This local path disclosure gate does not authorize media file opening." in text
    assert "This local path disclosure gate does not authorize media execution." in text
    assert "This local path disclosure gate only defines the sanitized local path disclosure boundary." in text


def test_required_checks_reference_previous_gates_and_generic_guards():
    text = _text()
    required_checks = [
        "This local path disclosure gate test.",
...
def test_closing_state_is_ready_for_real_file_access_readiness_gate():
    text = _text()
    assert "LOCAL_PATH_DISCLOSURE_BOUNDARY_READY_FOR_REAL_FILE_ACCESS_READINESS_GATE" in text
