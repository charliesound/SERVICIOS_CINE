from pathlib import Path

QA_DOC = Path(
    "docs/product/local_media_agent/"
    "cid_local_media_agent_ffprobe_controlled_file_metadata_visible_report_renderer_implementation_qa_gate_v1.md"
)
RUNTIME = Path("scripts/local_media_agent/ffprobe_controlled_file_metadata_visible_report_renderer.py")
SOURCE_DOC = Path(
    "docs/product/local_media_agent/"
    "cid_local_media_agent_ffprobe_controlled_file_metadata_visible_report_renderer_implementation_v1.md"
)
SOURCE_TEST = Path(
    "tests/unit/"
    "test_cid_local_media_agent_ffprobe_controlled_file_metadata_visible_report_renderer_implementation.py"
)
CONTRACT_QA_DOC = Path(
    "docs/product/local_media_agent/"
    "cid_local_media_agent_ffprobe_controlled_file_metadata_visible_report_renderer_implementation_contract_qa_gate_v1.md"
)
CONTRACT_QA_TEST = Path(
    "tests/unit/"
    "test_cid_local_media_agent_ffprobe_controlled_file_metadata_visible_report_renderer_implementation_contract_qa_gate.py"
)

QA_PHASE = (
    "CID.LOCAL_MEDIA_AGENT.FFPROBE.CONTROLLED.FILE.METADATA.PREFLIGHT."
    "VISIBLE.REPORT.RENDERER.IMPLEMENTATION.QA.GATE.V1"
)
SOURCE_PHASE = (
    "CID.LOCAL_MEDIA_AGENT.FFPROBE.CONTROLLED.FILE.METADATA.PREFLIGHT."
    "VISIBLE.REPORT.RENDERER.IMPLEMENTATION.V1"
)
NEXT_PHASE = (
    "CID.LOCAL_MEDIA_AGENT.FFPROBE.CONTROLLED.FILE.METADATA.PREFLIGHT."
    "VISIBLE.REPORT.RENDERER.CLI.CONTRACT.V1"
)


def read(path: Path) -> str:
    return path.read_text(encoding="utf-8")


def assert_all_present(content: str, required: list[str]) -> None:
    for item in required:
        assert item in content


def test_qa_gate_phase_and_result_are_declared() -> None:
    assert_all_present(read(QA_DOC), [
        QA_PHASE,
        "PASS_FFPROBE_CONTROLLED_FILE_METADATA_VISIBLE_REPORT_RENDERER_IMPLEMENTATION_VALIDATED",
        "LOCAL_MEDIA_AGENT_FFPROBE_CONTROLLED_FILE_METADATA_VISIBLE_REPORT_RENDERER_IMPLEMENTATION_QA_GATE_PASS_CLOSED",
    ])


def test_source_stable_state_is_declared() -> None:
    assert_all_present(read(QA_DOC), [
        "f46c842a90f3f08705654be542134d54a9829ad8",
        "feat: add CID Local Media Agent ffprobe visible report renderer implementation",
        "cid-dev-stable-local-media-agent-ffprobe-controlled-file-metadata-visible-report-renderer-implementation-v1-20260622",
        SOURCE_PHASE,
    ])


def test_required_source_files_exist() -> None:
    for path in [
        QA_DOC,
        RUNTIME,
        SOURCE_DOC,
        SOURCE_TEST,
        CONTRACT_QA_DOC,
        CONTRACT_QA_TEST,
    ]:
        assert path.exists(), path


def test_runtime_api_is_present() -> None:
    assert_all_present(read(RUNTIME), [
        "def render_controlled_ffprobe_metadata_visible_report(payload: dict) -> str:",
    ])


def test_required_runtime_behavior_is_declared() -> None:
    assert_all_present(read(QA_DOC), [
        "accept only already-safe controlled metadata payloads",
        "`input_policy` equal to `controlled_fixture_only`",
        "`ffprobe_command_kind` equal to `metadata_json`",
        "`input_path_redacted` only",
        "redact unsafe filenames",
        "never emit full local paths",
        "`metadata.format` when null",
        "`metadata.streams` as a list",
        "render deterministic output",
        "plain text or Markdown-safe text",
        "render report title",
        "render phase",
        "render input policy",
        "render redacted input filename",
        "render preflight result",
        "render format summary",
        "render stream summary",
        "render video stream summary",
        "render audio stream summary",
        "render safety boundary summary",
        "render blocked operations summary",
        "render human review required note",
        "render next safe phase",
        "render safe failure reports",
        "invalid input policy",
        "invalid command kind",
        "empty streams",
        "unknown stream types",
        "blocked operation flags false",
    ])


def test_required_runtime_boundaries_are_declared() -> None:
    assert_all_present(read(QA_DOC), [
        "does not:",
        "import forbidden networking or backend libraries",
        "use subprocess",
        "execute ffmpeg",
        "execute ffprobe",
        "scan folders",
        "read real media files",
        "extract audio",
        "perform sync",
        "perform transcription",
        "create subtitles",
        "export timelines",
        "perform SaaS upload",
        "write to database",
        "create an installer",
        "authorize client-facing use",
        "authorize public demo",
        "authorize sales demo",
        "authorize production use",
    ])


def test_source_implementation_declares_expected_result_and_next_phase() -> None:
    assert_all_present(read(SOURCE_DOC), [
        SOURCE_PHASE,
        "LOCAL_MEDIA_AGENT_FFPROBE_CONTROLLED_FILE_METADATA_VISIBLE_REPORT_RENDERER_IMPLEMENTATION_PASS_READY_FOR_QA_GATE",
        "CID.LOCAL_MEDIA_AGENT.FFPROBE.CONTROLLED.FILE.METADATA.PREFLIGHT.VISIBLE.REPORT.RENDERER.IMPLEMENTATION.QA.GATE.V1",
    ])


def test_source_implementation_test_mentions_runtime_contract_requirements() -> None:
    assert_all_present(read(SOURCE_TEST), [
        "render_controlled_ffprobe_metadata_visible_report",
        "input_path_redacted",
        "controlled_fixture_only",
        "metadata_json",
        "Human Review Required",
        "Blocked Operations",
        "deterministic",
    ])


def test_validation_evidence_and_next_safe_phase_are_declared() -> None:
    assert_all_present(read(QA_DOC), [
        "visible report renderer implementation QA gate test passing",
        "visible report renderer implementation test passing",
        "visible report renderer implementation contract QA gate test passing",
        "py_compile passing for runtime and tests",
        "no forbidden runtime imports",
        "no subprocess or process execution pattern",
        "no ffmpeg or ffprobe execution pattern",
        NEXT_PHASE,
    ])


def test_no_wrong_phase_prefix_is_present() -> None:
    for path in [QA_DOC, RUNTIME, SOURCE_DOC, SOURCE_TEST]:
        assert "CID.LOCAL_AGENT" not in read(path)


def test_runtime_has_no_forbidden_imports_or_process_execution() -> None:
    content = read(RUNTIME)
    denied_imports = [
        f"import {name}" for name in ["requests", "httpx", "socket", "sqlalchemy", "fastapi", "subprocess"]
    ] + [
        f"from {name}" for name in ["requests", "httpx", "socket", "sqlalchemy", "fastapi", "subprocess"]
    ]
    denied_runtime_patterns = [
        "subprocess.",
        "Popen(",
        "check_output(",
        "check_call(",
        '"ffmpeg"',
        "'ffmpeg'",
        '"ffprobe"',
        "'ffprobe'",
        "ffmpeg -",
        "ffprobe -",
    ]
    for item in [*denied_imports, *denied_runtime_patterns]:
        assert item not in content


def test_acceptance_result_and_next_phase_are_declared() -> None:
    assert_all_present(read(QA_DOC), [
        "LOCAL_MEDIA_AGENT_FFPROBE_CONTROLLED_FILE_METADATA_VISIBLE_REPORT_RENDERER_IMPLEMENTATION_QA_GATE_PASS_CLOSED",
        NEXT_PHASE,
    ])
