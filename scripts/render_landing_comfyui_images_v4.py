...
PAYLOADS_DIR = ROOT / ".tmp" / "landing_comfyui_v4" / "image_payloads"
JOBS_PATH = ROOT / ".tmp" / "landing_comfyui_v4" / "image_render_jobs.json"
...
def submit_prompt(workflow: dict[str, Any]) -> dict[str, Any]:
    body = json.dumps(
        {
            "prompt": workflow,
            "client_id": f"landing-v4-images-{uuid.uuid4().hex}",
        }
    ).encode("utf-8")
...
def main() -> int:
    if os.environ.get("ENABLE_REAL_COMFYUI_RENDER") != "1":
        print("Render bloqueado: ENABLE_REAL_COMFYUI_RENDER no está activado.")
        print("Para renderizar real: ENABLE_REAL_COMFYUI_RENDER=1 python3 scripts/render_landing_comfyui_images_v4.py")
        return 0

    payloads = load_payloads()
    if not payloads:
        raise RuntimeError("No image payloads found. Run scripts/build_landing_comfyui_payloads_v4.py first.")
    ...
    for payload in payloads:
        try:
            ...
            print(f"  QUEUED: {payload['image_key']} -> prompt_id={response.get('prompt_id')}")
            ...
            print(f"  HTTP ERROR: {payload['image_key']} ({exc.code})")
            ...
            print(f"  CONNECTION ERROR: {payload['image_key']} -> {exc.reason}")
