...
class ProjectScriptAnalysisFlowIntegrationTest(unittest.TestCase):
    ...
    def test_sec_prefixed_script_analysis_detects_scenes_sequences_and_character(self) -> None:
        ...
        try:
            ...
            self.assertEqual(payload["scenes"][0]["normalized_heading"], "INT. CASA ABANDONADA - NOCHE")
            self.assertEqual(payload["scenes"][0]["source_sequence_number"], 1)
            self.assertEqual(payload["scenes"][0]["source_sequence_label"], "Sec 1")
            self.assertEqual(payload["scenes"][0]["scene_type"], "INT")
            self.assertEqual(payload["scenes"][0]["location"], "CASA ABANDONADA")
            self.assertEqual(payload["scenes"][0]["time_of_day"], "NOCHE")
            self.assertEqual(payload["scenes"][1]["source_sequence_number"], 3)
            self.assertEqual(payload["scenes"][1]["scene_type"], "EXT")
            self.assertEqual(payload["scenes"][1]["location"], "BOSQUE")
