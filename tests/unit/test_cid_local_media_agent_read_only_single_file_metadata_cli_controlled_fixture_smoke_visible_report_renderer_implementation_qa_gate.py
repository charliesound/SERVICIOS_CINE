...
DOC = ROOT / "docs/product/local_media_agent/cid_local_media_agent_read_only_single_file_metadata_cli_controlled_fixture_smoke_visible_report_renderer_implementation_qa_gate_v1.md"
RENDERER = ROOT / "scripts/local_media_agent/controlled_fixture_smoke_visible_report_renderer.py"
IMPLEMENTATION_DOC = ROOT / "docs/product/local_media_agent/cid_local_media_agent_read_only_single_file_metadata_cli_controlled_fixture_smoke_visible_report_renderer_implementation_gate_v1.md"
IMPLEMENTATION_TEST = ROOT / "tests/unit/test_cid_local_media_agent_read_only_single_file_metadata_cli_controlled_fixture_smoke_visible_report_renderer_implementation_gate.py"
...
PHASE = "CID.LOCAL_MEDIA_AGENT.READ_ONLY.SINGLE_FILE.METADATA.CLI.CONTROLLED_FIXTURE.SMOKE.VISIBLE_REPORT.RENDERER.IMPLEMENTATION.QA.GATE.V1"
RESULT = "LOCAL_MEDIA_AGENT_READ_ONLY_SINGLE_FILE_METADATA_CLI_CONTROLLED_FIXTURE_SMOKE_VISIBLE_REPORT_RENDERER_IMPLEMENTATION_QA_GATE_V1_CLOSED"
BASE_HEAD = "927cd288f49b5578264b13e7b6442a801847fa47"
BASE_TAG = "cid-dev-stable-local-media-agent-read-only-single-file-metadata-cli-controlled-fixture-smoke-visible-report-renderer-implementation-gate-v1-20260701"
EXPECTED_BYTES = 239
...
def load_renderer_module():
    spec = importlib.util.spec_from_file_location("cid_lma_renderer_qa", RENDERER)
...
def test_qa_document_declares_phase_result_base_and_scope() -> None:
    ...
    required = [
        ...
        "does not add renderer behavior",
...
def test_qa_document_references_prior_sources() -> None:
    for path in [IMPLEMENTATION_DOC, IMPLEMENTATION_TEST, READINESS_DOC, READINESS_TEST, CONTRACT_DOC, CONTRACT_TEST]:
...
def test_renderer_is_parseable_and_exposes_expected_function() -> None:
...
def test_renderer_imports_remain_minimal_and_safe() -> None:
    roots = import_roots(parse_python(RENDERER))
    assert roots <= {"__future__", "collections", "typing"}


def test_renderer_does_not_import_forbidden_runtime_dependencies() -> None:
    roots = import_roots(parse_python(RENDERER))
    forbidden = {
...
def test_renderer_has_no_file_process_scan_or_media_probe_calls() -> None:
...
def test_renderer_returns_markdown_in_memory_and_does_not_mutate_input() -> None:
    module = load_renderer_module()
...
def test_renderer_redacts_obvious_private_paths() -> None:
    module = load_renderer_module()
    smoke_result = sample_smoke_result()
    smoke_result["fixture_root"] = "/home/private_user/secret/project"
    smoke_result["file_name"] = "C:\\\\private\\\\customer\\\\file.mov"
    report = module.render_controlled_fixture_smoke_visible_report(smoke_result)
...
def test_renderer_rejects_non_mapping_input() -> None:
    module = load_renderer_module()
    try:
        module.render_controlled_fixture_smoke_visible_report(["not", "a", "mapping"])
    except TypeError as exc:
        assert "smoke_result must be a mapping" in str(exc)
    else:
        raise AssertionError("renderer accepted non-mapping input")
