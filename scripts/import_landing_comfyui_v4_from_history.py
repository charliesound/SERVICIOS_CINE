...
MANIFEST_PATH = ROOT / "src_frontend" / "public" / "landing-media" / "_landing_v4_generated_manifest.txt"
...
def main() -> int:
    ...
    for job in queued_jobs:
        ...
        print(f"  {image_key} -> prompt_id={prompt_id}")
        ...
        if not filename:
            ...
            print(f"    NO FILENAME in outputs: {json.dumps(outputs, indent=2)[:200]}")
            continue

        subfolder = f"landing_v4/{image_key}"
        output_path = MEDIA_DIR / target_file
        if output_path.exists():
            print(f"    EXISTS (skipped): {target_file}")
            manifest_lines.append(f"{image_key}\t{target_file}\tstatus=exists")
            continue

        success = download_image(filename, subfolder, output_path)
        if success:
            print(f"    IMPORTED: {target_file} (from {filename})")
            manifest_lines.append(f"{image_key}\t{target_file}\tstatus=imported\tsource={filename}")
        else:
