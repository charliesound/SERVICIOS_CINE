...
def test_input_folder_contract_declares_phase_baseline_and_decision() -> None:
    text = _read(INPUT_FOLDER_DOC)

    assert "CID.LOCAL_MEDIA_AGENT.REAL.INPUT.FOLDER.CONTRACT.V1" in text
    assert "Stable HEAD before this phase: `101f267`." in text
    ...
    assert "REAL_INPUT_FOLDER_CONTRACT_READY_FOR_QA_GATE_WITH_RESTRICTIONS" in text


def test_input_folder_contract_blocks_real_execution_and_runtime_changes() -> None:
    text = _read(INPUT_FOLDER_DOC)
...
def test_input_folder_contract_references_previous_privacy_and_scope_gates() -> None:
    text = _read(INPUT_FOLDER_DOC)
...
def test_input_folder_contract_defines_manual_local_lab_folder_position() -> None:
    text = _read(INPUT_FOLDER_DOC)
...
def test_input_folder_contract_requires_local_only_folder_rules() -> None:
    text = _read(INPUT_FOLDER_DOC)
...
def test_input_folder_contract_sets_first_real_test_size_limits() -> None:
    text = _read(INPUT_FOLDER_DOC)
...
def test_input_folder_contract_sets_first_real_test_extension_allowlist() -> None:
    text = _read(INPUT_FOLDER_DOC)
...
def test_input_folder_contract_requires_output_folder_separation() -> None:
    text = _read(INPUT_FOLDER_DOC)
...
def test_input_folder_contract_preserves_path_privacy() -> None:
    text = _read(INPUT_FOLDER_DOC)
...
def test_input_folder_contract_requires_future_gates_before_real_execution() -> None:
    text = _read(INPUT_FOLDER_DOC)
...
def test_input_folder_contract_explicitly_blocks_current_phase_actions() -> None:
    text = _read(INPUT_FOLDER_DOC)
...
def test_previous_privacy_gate_still_blocks_upload_cloud_and_sensitive_paths() -> None:
...
def test_previous_scope_contracts_still_restrict_real_material() -> None:
...
def test_runtime_sources_are_not_modified_for_input_folder_contract() -> None:
    sources = [
        _read(SYNTHETIC_CLI),
    ...
    forbidden_runtime_terms = [
        "REAL.INPUT.FOLDER.CONTRACT",
        "REAL_INPUT_FOLDER_CONTRACT_READY",
        "maximum file count: 25 media files",
        "maximum total selected media size: 10 GB",
        "real media execution",
        "client delivery",
