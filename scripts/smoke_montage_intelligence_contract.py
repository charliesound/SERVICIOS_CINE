...
def validate_contract(base_url: str) -> None:
    status, profiles_payload = request_json("GET", f"{base_url}/api/cid/script-to-prompt/montage-profiles")
    if status != 200 or not isinstance(profiles_payload, list) or not profiles_payload:
        raise RuntimeError("Montage profiles endpoint did not return profiles")
    ...
    if not intents or not prompts or not validations:
        raise RuntimeError("Missing intents/prompts/validations in run payload")

    first_intent = intents[0]
    first_prompt = prompts[0]
    first_validation = validations[0]
    if not first_intent.get("directorial_intent"):
        raise RuntimeError("Missing directorial_intent")
    if not first_intent.get("montage_intent"):
        raise RuntimeError("Missing montage_intent")
    if not first_intent.get("editorial_beats"):
        raise RuntimeError("Missing editorial_beats")

    positive_prompt = str(first_prompt.get("positive_prompt", ""))
    if "in the style of" in positive_prompt.lower():
        raise RuntimeError("Prompt contains forbidden style reference")
    if "editorial function:" not in positive_prompt.lower():
        raise RuntimeError("Prompt missing editorial function")
    if not any(token in positive_prompt.lower() for token in ["cut reason:", "visual continuity:", "eyeline strategy:"]):
        raise RuntimeError("Prompt missing montage continuity or cut reason")
    if float(first_validation.get("score", 0.0) or 0.0) < 0.65:
        raise RuntimeError("Validation score below montage threshold")
...
def main() -> int:
    ...
    try:
        ...
        print("PASS: Montage intelligence contract smoke test")
        return 0
    finally:
