...
DOC = ROOT / "docs/product/local_media_agent/cid_local_media_agent_real_media_preflight_code_skeleton_readiness_gate_v1.md"
...
def test_code_skeleton_readiness_gate_doc_exists():
    assert DOC.exists()


def test_phase_identifier_is_present():
    text = _text()
    assert "CID.LOCAL_MEDIA_AGENT.REAL_MEDIA_PREFLIGHT.CODE_SKELETON.READINESS.GATE.V1" in text


def test_expected_closure_result_is_present():
    text = _text()
    assert "LOCAL_MEDIA_AGENT_REAL_MEDIA_PREFLIGHT_CODE_SKELETON_READINESS_GATE_V1_CLOSED" in text


def test_starting_state_is_from_isolated_implementation_gate():
    text = _text()
    assert "ISOLATED_IMPLEMENTATION_BOUNDARY_READY_FOR_CODE_SKELETON_READINESS_GATE" in text


def test_target_state_is_ready_for_code_skeleton_gate():
    text = _text()
    assert "READY_FOR_CODE_SKELETON_GATE" in text


def test_gate_purpose_is_readiness_only():
    text = _text()
    assert "This readiness gate prepares the conditions for a later isolated code skeleton gate." in text
    assert "This gate does not create code skeleton files." in text
...
def test_source_isolated_implementation_gate_is_referenced():
    text = _text()
...
def test_source_isolated_implementation_boundary_record_values_are_preserved():
    text = _text()
    required_values = [
...
def test_code_skeleton_readiness_record_is_declared():
    text = _text()
    required_fields = [
        "CODE_SKELETON_READINESS_RECORD_ID",
        "CODE_SKELETON_INPUT_RECORD_ID",
        "CODE_SKELETON_SOURCE_ISOLATED_IMPLEMENTATION_BOUNDARY_RECORD_ID",
        "CODE_SKELETON_SOURCE_ISOLATED_IMPLEMENTATION_BOUNDARY_HANDLE",
        "CODE_SKELETON_SOURCE_REAL_STAT_IMPLEMENTATION_CONTRACT_RECORD_ID",
        "CODE_SKELETON_SOURCE_REAL_STAT_IMPLEMENTATION_CONTRACT_HANDLE",
        "CODE_SKELETON_SOURCE_STAT_EXECUTION_BOUNDARY_RECORD_ID",
        "CODE_SKELETON_SOURCE_STAT_EXECUTION_BOUNDARY_HANDLE",
        "CODE_SKELETON_SOURCE_CONTROLLED_STAT_BOUNDARY_RECORD_ID",
        "CODE_SKELETON_SOURCE_CONTROLLED_STAT_BOUNDARY_HANDLE",
        "CODE_SKELETON_SOURCE_REAL_FILE_ACCESS_BOUNDARY_RECORD_ID",
        "CODE_SKELETON_SOURCE_REAL_FILE_ACCESS_BOUNDARY_HANDLE",
        "CODE_SKELETON_SOURCE_LOCAL_PATH_DISCLOSURE_BOUNDARY_RECORD_ID",
        "CODE_SKELETON_SOURCE_LOCAL_PATH_DISCLOSURE_BOUNDARY_HANDLE",
        "CODE_SKELETON_SOURCE_SELECTION_BOUNDARY_RECORD_ID",
        "CODE_SKELETON_SOURCE_SELECTION_BOUNDARY_HANDLE",
        "CODE_SKELETON_SOURCE_CONFIRMATION_RECORD_ID",
        "CODE_SKELETON_SOURCE_CONFIRMATION_HANDLE",
        "CODE_SKELETON_SOURCE_CONFIRMATION_VALUE",
        "CODE_SKELETON_SOURCE_TOKEN_RECORD_ID",
        "CODE_SKELETON_SOURCE_TOKEN_HANDLE",
        "CODE_SKELETON_SANITIZED_SELECTION_TOKEN",
        "CODE_SKELETON_SOURCE_REFERENCE_RECORD_ID",
        "CODE_SKELETON_SOURCE_REFERENCE_HANDLE",
        "CODE_SKELETON_SOURCE_EVENT_ID",
        "CODE_SKELETON_SOURCE_EVENT_HANDLE",
        "CODE_SKELETON_VERDICT",
    ]
    for field in required_fields:
        assert field in text


def test_code_skeleton_readiness_values_are_sanitized():
    text = _text()
    required_values = [
        ...
        "LOCAL_REFERENCE_HANDLE_001",
        "OPERATOR_LOCAL_SELECTION_EVENT_HANDLE_001",
        "ready_for_code_skeleton_gate_without_code_runtime_stat_or_metadata_read",
    ]
    for value in required_values:
        assert value in text


def test_readiness_status_is_ready_for_code_skeleton_gate():
    text = _text()
    assert "CODE_SKELETON_READINESS_STATUS" in text
    assert "ready_for_code_skeleton_gate" in text
    assert "The readiness status is `ready_for_code_skeleton_gate`." in text


def test_no_code_skeleton_or_runtime_is_created_in_this_gate():
    text = _text()
    required_statuses = [
        "CODE_SKELETON_CODE_STATUS",
        "not_created_in_this_gate",
        "CODE_SKELETON_IMPLEMENTATION_STATUS",
        "not_implemented_in_this_gate",
        "CODE_SKELETON_RUNTIME_STATUS",
        "no_runtime_created",
    ]
    for status in required_statuses:
        assert status in text
    assert "No code skeleton files are created in this gate." in text
    assert "No implementation runtime is created in this gate." in text


def test_existing_cli_runtime_is_not_modified():
    text = _text()
    assert "CODE_SKELETON_CLI_RUNTIME_STATUS" in text
...
def test_no_stat_execution_occurs_in_this_gate():
    text = _text()
    assert "CODE_SKELETON_STAT_STATUS" in text
...
def test_no_real_file_access_occurs_in_this_gate():
    text = _text()
    assert "CODE_SKELETON_ACCESS_STATUS" in text
...
def test_no_media_file_is_opened_in_this_gate():
    text = _text()
    assert "CODE_SKELETON_FILE_OPEN_STATUS" in text
...
def test_no_file_bytes_or_real_metadata_are_read():
    text = _text()
    required_statuses = [
        "CODE_SKELETON_FILE_BYTES_STATUS",
        "CODE_SKELETON_FILESYSTEM_METADATA_STATUS",
...
def test_no_file_size_timestamps_or_hashes_are_recorded():
    text = _text()
    required_statuses = [
        "CODE_SKELETON_FILE_SIZE_STATUS",
        "CODE_SKELETON_TIMESTAMP_STATUS",
        "CODE_SKELETON_HASH_STATUS",
...
def test_no_local_path_or_identity_metadata_is_committed():
    text = _text()
    required_statuses = [
        "CODE_SKELETON_REAL_PATH_STATUS",
        "CODE_SKELETON_ABSOLUTE_PATH_STATUS",
        "CODE_SKELETON_RELATIVE_PATH_STATUS",
        "not_recorded_in_committed_artifacts",
        "CODE_SKELETON_REAL_FILENAME_STATUS",
        "CODE_SKELETON_PARENT_FOLDER_STATUS",
        "not_recorded_in_this_gate",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_media_decode_probe_scan_or_transcription_occurs():
    text = _text()
    required_statuses = [
        "CODE_SKELETON_MEDIA_DECODE_STATUS",
        "CODE_SKELETON_MEDIA_PROBE_STATUS",
        "CODE_SKELETON_MEDIA_SCAN_STATUS",
        "CODE_SKELETON_TRANSCRIPTION_STATUS",
...
def test_no_thumbnails_or_waveforms_are_generated():
    text = _text()
    required_statuses = [
        "CODE_SKELETON_THUMBNAIL_STATUS",
        "not_generated_in_this_gate",
        "CODE_SKELETON_WAVEFORM_STATUS",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_execution_or_media_tools_are_run():
    text = _text()
    required_statuses = [
        "CODE_SKELETON_EXECUTION_STATUS",
        "CODE_SKELETON_FFMPEG_STATUS",
        "CODE_SKELETON_FFPROBE_STATUS",
        "CODE_SKELETON_SCANNER_STATUS",
        "not_executed_in_this_gate",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_saas_integration_is_created():
    text = _text()
    assert "CODE_SKELETON_SAAS_STATUS" in text
    assert "no_saas_integration" in text
    assert "No SaaS integration is created in this gate." in text


def test_code_skeleton_gate_constraints_are_conservative():
    text = _text()
    constraints = [
        ...
        "It must create only an isolated code skeleton if explicitly authorized by that later gate.",
        "It must not execute against a real file.",
...
def test_positive_assertions_preserve_scope():
    ...
    required_assertions = [
        ...
        "`code_skeleton_readiness_001` is created as a readiness record.",
        "`isolated_implementation_boundary_001` remains the source isolated implementation boundary.",
        "`ISOLATED_IMPLEMENTATION_BOUNDARY_HANDLE_001` remains a non-filesystem isolated implementation boundary handle.",
...
def test_next_phase_boundary_is_code_skeleton_gate_only():
    text = _text()
    assert "The next conservative phase may define a code skeleton gate." in text
    assert "This code skeleton readiness gate does not authorize code skeleton creation." in text
    assert "This code skeleton readiness gate does not authorize runtime implementation." in text
    assert "This code skeleton readiness gate does not authorize filesystem stat execution." in text
    assert "This code skeleton readiness gate does not authorize accessing a real file." in text
    assert "This code skeleton readiness gate does not authorize opening media." in text
    assert "This code skeleton readiness gate does not authorize reading file bytes." in text
    assert "This code skeleton readiness gate does not authorize reading real metadata." in text
    assert "This code skeleton readiness gate does not authorize media execution." in text
    assert "This code skeleton readiness gate only prepares conditions for a later code skeleton gate." in text


def test_required_checks_reference_previous_gates_and_generic_guards():
    text = _text()
    required_checks = [
        "This code skeleton readiness gate test.",
...
def test_closing_state_is_ready_for_code_skeleton_gate():
    text = _text()
    assert "READY_FOR_CODE_SKELETON_GATE" in text
