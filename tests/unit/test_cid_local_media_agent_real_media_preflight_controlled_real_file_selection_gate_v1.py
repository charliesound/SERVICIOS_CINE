...
DOC = ROOT / "docs/product/local_media_agent/cid_local_media_agent_real_media_preflight_controlled_real_file_selection_gate_v1.md"
...
def test_controlled_real_file_selection_gate_doc_exists():
    assert DOC.exists()
...
def test_starting_state_is_ready_for_controlled_real_file_selection_gate():
    text = _text()
    assert "READY_FOR_CONTROLLED_REAL_FILE_SELECTION_GATE" in text


def test_target_state_prepares_local_path_disclosure_readiness():
    text = _text()
...
def test_gate_purpose_defines_boundary_only():
    text = _text()
    assert "This gate defines the controlled real file selection boundary." in text
    assert "This gate records only a sanitized controlled selection boundary." in text
    assert "This gate does not perform actual filesystem selection." in text
    assert "This gate does not select a real file." in text
    assert "This gate is limited to documentation and tests." in text


def test_source_controlled_real_file_selection_readiness_gate_is_referenced():
    text = _text()
    assert "CID.LOCAL_MEDIA_AGENT.REAL_MEDIA_PREFLIGHT.CONTROLLED_REAL_FILE_SELECTION.READINESS.GATE.V1" in text
    assert "LOCAL_MEDIA_AGENT_REAL_MEDIA_PREFLIGHT_CONTROLLED_REAL_FILE_SELECTION_READINESS_GATE_V1_CLOSED" in text


def test_source_controlled_real_file_selection_readiness_record_values_are_preserved():
    text = _text()
    required_values = [
        ...
        "single_file_claimed",
        "ready_for_controlled_real_file_selection_gate_without_selection_or_filesystem_touch",
    ]
    for value in required_values:
        assert value in text


def test_controlled_real_file_selection_boundary_record_is_declared():
    text = _text()
    required_fields = [
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_RECORD_ID",
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_INPUT_RECORD_ID",
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_SOURCE_READINESS_RECORD_ID",
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_SOURCE_CONFIRMATION_RECORD_ID",
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_SOURCE_CONFIRMATION_HANDLE",
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_SOURCE_CONFIRMATION_VALUE",
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_SOURCE_TOKEN_RECORD_ID",
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_SOURCE_TOKEN_HANDLE",
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_SANITIZED_SELECTION_TOKEN",
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_SOURCE_REFERENCE_RECORD_ID",
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_SOURCE_REFERENCE_HANDLE",
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_SOURCE_EVENT_ID",
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_SOURCE_EVENT_HANDLE",
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_HANDLE",
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_VERDICT",
    ]
    for field in required_fields:
        assert field in text


def test_controlled_real_file_selection_boundary_values_are_sanitized():
    text = _text()
    required_values = [
        "controlled_real_file_selection_boundary_001",
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_HANDLE_001",
        "defined_as_sanitized_selection_boundary",
        "prepares_local_path_disclosure_readiness_only",
...
def test_boundary_status_and_scope_are_limited():
    text = _text()
    assert "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_STATUS" in text
    assert "defined_as_sanitized_selection_boundary" in text
    assert "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_SCOPE_STATUS" in text
    assert "prepares_local_path_disclosure_readiness_only" in text
    assert "The boundary status is `defined_as_sanitized_selection_boundary`." in text
    assert "The boundary scope prepares local path disclosure readiness only." in text


def test_no_real_file_selection_occurs():
    text = _text()
    assert "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_REAL_SELECTION_STATUS" in text
...
def test_no_real_path_is_resolved_or_recorded():
    text = _text()
    assert "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_REAL_PATH_STATUS" in text
    assert "not_resolved_or_recorded" in text
    assert "No real path is resolved or recorded." in text


def test_no_filename_or_parent_folder_is_recorded():
    text = _text()
    required_statuses = [
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_REAL_FILENAME_STATUS",
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_PARENT_FOLDER_STATUS",
        "not_recorded",
    ]
...
def test_no_file_size_timestamps_or_hashes_are_recorded():
    text = _text()
    required_statuses = [
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_FILE_SIZE_STATUS",
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_TIMESTAMP_STATUS",
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_HASH_STATUS",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_filesystem_or_file_open_operations_occur():
    text = _text()
    required_statuses = [
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_FILESYSTEM_METADATA_STATUS",
        "not_read",
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_FILE_OPEN_STATUS",
        "not_opened",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_execution_or_media_tools_are_run():
    text = _text()
    required_statuses = [
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_EXECUTION_STATUS",
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_FFMPEG_STATUS",
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_FFPROBE_STATUS",
        "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_SCANNER_STATUS",
        "not_executed",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_runtime_or_saas_integration_is_created():
    text = _text()
    assert "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_RUNTIME_STATUS" in text
    assert "no_runtime_created" in text
    assert "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_SAAS_STATUS" in text
...
def test_positive_assertions_preserve_scope():
    ...
    required_assertions = [
        ...
        "`controlled_real_file_selection_boundary_001` is created as a sanitized boundary record.",
        "`CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_HANDLE_001` is a non-filesystem boundary handle.",
        "`controlled_real_file_selection_readiness_001` remains the source readiness record.",
...
def test_later_local_path_disclosure_readiness_constraints_are_conservative():
    text = _text()
    constraints = [
...
def test_file_selection_path_resolution_and_disclosure_are_forbidden():
    text = _text()
    forbidden_items = [
...
def test_next_phase_boundary_is_local_path_disclosure_readiness_only():
    text = _text()
    assert "The next conservative phase may prepare a local path disclosure readiness gate." in text
    assert "This controlled real file selection gate does not authorize real file selection." in text
    assert "This controlled real file selection gate does not authorize path resolution." in text
    assert "This controlled real file selection gate does not authorize local path disclosure." in text
    assert "This controlled real file selection gate does not authorize filesystem access." in text
    assert "This controlled real file selection gate does not authorize media execution." in text
    assert "This controlled real file selection gate only defines the sanitized controlled selection boundary." in text


def test_required_checks_reference_previous_gates_and_generic_guards():
    text = _text()
    required_checks = [
        "This controlled real file selection gate test.",
...
def test_closing_state_is_ready_for_local_path_disclosure_readiness_gate():
    text = _text()
    assert "CONTROLLED_REAL_FILE_SELECTION_BOUNDARY_READY_FOR_LOCAL_PATH_DISCLOSURE_READINESS_GATE" in text
