...
DOC = ROOT / "docs/product/local_media_agent/cid_local_media_agent_real_media_preflight_isolated_implementation_readiness_gate_v1.md"
...
def test_isolated_implementation_readiness_gate_doc_exists():
    assert DOC.exists()
...
def test_starting_state_is_from_real_stat_implementation_gate():
    text = _text()
    assert "REAL_STAT_IMPLEMENTATION_CONTRACT_READY_FOR_ISOLATED_IMPLEMENTATION_READINESS_GATE" in text


def test_target_state_is_ready_for_isolated_implementation_gate():
    text = _text()
...
def test_gate_purpose_is_readiness_only():
    text = _text()
    assert "This readiness gate prepares the conditions for a later isolated implementation gate." in text
...
def test_source_real_stat_implementation_gate_is_referenced():
    text = _text()
...
def test_source_real_stat_implementation_contract_values_are_preserved():
    text = _text()
    required_values = [
...
def test_isolated_implementation_readiness_record_is_declared():
    text = _text()
    required_fields = [
        "ISOLATED_IMPLEMENTATION_READINESS_RECORD_ID",
        "ISOLATED_IMPLEMENTATION_INPUT_RECORD_ID",
        "ISOLATED_IMPLEMENTATION_SOURCE_REAL_STAT_IMPLEMENTATION_CONTRACT_RECORD_ID",
        "ISOLATED_IMPLEMENTATION_SOURCE_REAL_STAT_IMPLEMENTATION_CONTRACT_HANDLE",
        "ISOLATED_IMPLEMENTATION_SOURCE_STAT_EXECUTION_BOUNDARY_RECORD_ID",
        "ISOLATED_IMPLEMENTATION_SOURCE_STAT_EXECUTION_BOUNDARY_HANDLE",
        "ISOLATED_IMPLEMENTATION_SOURCE_CONTROLLED_STAT_BOUNDARY_RECORD_ID",
        "ISOLATED_IMPLEMENTATION_SOURCE_CONTROLLED_STAT_BOUNDARY_HANDLE",
        "ISOLATED_IMPLEMENTATION_SOURCE_REAL_FILE_ACCESS_BOUNDARY_RECORD_ID",
        "ISOLATED_IMPLEMENTATION_SOURCE_REAL_FILE_ACCESS_BOUNDARY_HANDLE",
        "ISOLATED_IMPLEMENTATION_SOURCE_LOCAL_PATH_DISCLOSURE_BOUNDARY_RECORD_ID",
        "ISOLATED_IMPLEMENTATION_SOURCE_LOCAL_PATH_DISCLOSURE_BOUNDARY_HANDLE",
        "ISOLATED_IMPLEMENTATION_SOURCE_SELECTION_BOUNDARY_RECORD_ID",
        "ISOLATED_IMPLEMENTATION_SOURCE_SELECTION_BOUNDARY_HANDLE",
        "ISOLATED_IMPLEMENTATION_SOURCE_CONFIRMATION_RECORD_ID",
        "ISOLATED_IMPLEMENTATION_SOURCE_CONFIRMATION_HANDLE",
        "ISOLATED_IMPLEMENTATION_SOURCE_CONFIRMATION_VALUE",
        "ISOLATED_IMPLEMENTATION_SOURCE_TOKEN_RECORD_ID",
        "ISOLATED_IMPLEMENTATION_SOURCE_TOKEN_HANDLE",
        "ISOLATED_IMPLEMENTATION_SANITIZED_SELECTION_TOKEN",
        "ISOLATED_IMPLEMENTATION_SOURCE_REFERENCE_RECORD_ID",
        "ISOLATED_IMPLEMENTATION_SOURCE_REFERENCE_HANDLE",
        "ISOLATED_IMPLEMENTATION_SOURCE_EVENT_ID",
        "ISOLATED_IMPLEMENTATION_SOURCE_EVENT_HANDLE",
        "ISOLATED_IMPLEMENTATION_VERDICT",
    ]
    for field in required_fields:
        assert field in text


def test_isolated_implementation_readiness_values_are_sanitized():
    text = _text()
    required_values = [
...
def test_readiness_status_is_ready_for_isolated_implementation_gate():
    text = _text()
    assert "ISOLATED_IMPLEMENTATION_READINESS_STATUS" in text
    assert "ready_for_isolated_implementation_gate" in text
    assert "The readiness status is `ready_for_isolated_implementation_gate`." in text


def test_no_implementation_or_runtime_is_created_in_this_gate():
    text = _text()
    assert "ISOLATED_IMPLEMENTATION_IMPLEMENTATION_STATUS" in text
    assert "not_implemented_in_this_gate" in text
    assert "ISOLATED_IMPLEMENTATION_RUNTIME_STATUS" in text
...
def test_existing_cli_runtime_is_not_modified():
    text = _text()
    assert "ISOLATED_IMPLEMENTATION_CLI_RUNTIME_STATUS" in text
...
def test_no_stat_execution_occurs_in_this_gate():
    text = _text()
    assert "ISOLATED_IMPLEMENTATION_STAT_STATUS" in text
...
def test_no_real_file_access_occurs_in_this_gate():
    text = _text()
    assert "ISOLATED_IMPLEMENTATION_ACCESS_STATUS" in text
...
def test_no_media_file_is_opened_in_this_gate():
    text = _text()
    assert "ISOLATED_IMPLEMENTATION_FILE_OPEN_STATUS" in text
...
def test_no_file_bytes_or_real_metadata_are_read():
    text = _text()
    required_statuses = [
        "ISOLATED_IMPLEMENTATION_FILE_BYTES_STATUS",
        "ISOLATED_IMPLEMENTATION_FILESYSTEM_METADATA_STATUS",
...
def test_no_file_size_timestamps_or_hashes_are_recorded():
    text = _text()
    required_statuses = [
        "ISOLATED_IMPLEMENTATION_FILE_SIZE_STATUS",
        "ISOLATED_IMPLEMENTATION_TIMESTAMP_STATUS",
        "ISOLATED_IMPLEMENTATION_HASH_STATUS",
...
def test_no_local_path_or_identity_metadata_is_committed():
    text = _text()
    required_statuses = [
        "ISOLATED_IMPLEMENTATION_REAL_PATH_STATUS",
        "ISOLATED_IMPLEMENTATION_ABSOLUTE_PATH_STATUS",
        "ISOLATED_IMPLEMENTATION_RELATIVE_PATH_STATUS",
        "not_recorded_in_committed_artifacts",
        "ISOLATED_IMPLEMENTATION_REAL_FILENAME_STATUS",
        "ISOLATED_IMPLEMENTATION_PARENT_FOLDER_STATUS",
        "not_recorded_in_this_gate",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_media_decode_probe_scan_or_transcription_occurs():
    text = _text()
    required_statuses = [
        "ISOLATED_IMPLEMENTATION_MEDIA_DECODE_STATUS",
        "ISOLATED_IMPLEMENTATION_MEDIA_PROBE_STATUS",
        "ISOLATED_IMPLEMENTATION_MEDIA_SCAN_STATUS",
        "ISOLATED_IMPLEMENTATION_TRANSCRIPTION_STATUS",
...
def test_no_thumbnails_or_waveforms_are_generated():
    text = _text()
    required_statuses = [
        "ISOLATED_IMPLEMENTATION_THUMBNAIL_STATUS",
        "not_generated_in_this_gate",
        "ISOLATED_IMPLEMENTATION_WAVEFORM_STATUS",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_execution_or_media_tools_are_run():
    text = _text()
    required_statuses = [
        "ISOLATED_IMPLEMENTATION_EXECUTION_STATUS",
        "ISOLATED_IMPLEMENTATION_FFMPEG_STATUS",
        "ISOLATED_IMPLEMENTATION_FFPROBE_STATUS",
        "ISOLATED_IMPLEMENTATION_SCANNER_STATUS",
        "not_executed_in_this_gate",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_saas_integration_is_created():
    text = _text()
    assert "ISOLATED_IMPLEMENTATION_SAAS_STATUS" in text
    assert "no_saas_integration" in text
    assert "No SaaS integration is created in this gate." in text


def test_isolated_implementation_gate_constraints_are_conservative():
    text = _text()
    constraints = [
        ...
        "It must define isolated implementation behavior without executing against a real file.",
...
def test_positive_assertions_preserve_scope():
    ...
    required_assertions = [
        ...
        "`isolated_implementation_readiness_001` is created as a readiness record.",
...
def test_next_phase_boundary_is_isolated_implementation_gate_only():
    text = _text()
    assert "The next conservative phase may define an isolated implementation gate." in text
    assert "This isolated implementation readiness gate does not authorize runtime implementation." in text
    assert "This isolated implementation readiness gate does not authorize filesystem stat execution." in text
    assert "This isolated implementation readiness gate does not authorize accessing a real file." in text
    assert "This isolated implementation readiness gate does not authorize opening media." in text
    assert "This isolated implementation readiness gate does not authorize reading file bytes." in text
    assert "This isolated implementation readiness gate does not authorize reading real metadata." in text
    assert "This isolated implementation readiness gate does not authorize media execution." in text
    assert "This isolated implementation readiness gate only prepares conditions for a later isolated implementation gate." in text


def test_required_checks_reference_previous_gates_and_generic_guards():
    text = _text()
    required_checks = [
        "This isolated implementation readiness gate test.",
...
def test_closing_state_is_ready_for_isolated_implementation_gate():
    text = _text()
    assert "READY_FOR_ISOLATED_IMPLEMENTATION_GATE" in text
