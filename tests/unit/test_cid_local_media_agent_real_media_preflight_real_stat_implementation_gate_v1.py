...
DOC = ROOT / "docs/product/local_media_agent/cid_local_media_agent_real_media_preflight_real_stat_implementation_gate_v1.md"
...
def test_real_stat_implementation_gate_doc_exists():
    assert DOC.exists()


def test_phase_identifier_is_present():
    text = _text()
    assert "CID.LOCAL_MEDIA_AGENT.REAL_MEDIA_PREFLIGHT.REAL_STAT_IMPLEMENTATION.GATE.V1" in text


def test_expected_closure_result_is_present():
    text = _text()
    assert "LOCAL_MEDIA_AGENT_REAL_MEDIA_PREFLIGHT_REAL_STAT_IMPLEMENTATION_GATE_V1_CLOSED" in text


def test_starting_state_is_ready_for_real_stat_implementation_gate():
    text = _text()
    assert "READY_FOR_REAL_STAT_IMPLEMENTATION_GATE" in text


def test_target_state_prepares_isolated_implementation_readiness():
    text = _text()
    assert "REAL_STAT_IMPLEMENTATION_CONTRACT_READY_FOR_ISOLATED_IMPLEMENTATION_READINESS_GATE" in text


def test_gate_purpose_defines_contract_only():
    text = _text()
    assert "This gate defines the isolated implementation contract for controlled filesystem stat inspection." in text
    assert "This gate creates only a sanitized real stat implementation contract record." in text
...
def test_source_real_stat_implementation_readiness_gate_is_referenced():
    text = _text()
...
def test_source_real_stat_implementation_readiness_record_values_are_preserved():
    text = _text()
    required_values = [
...
def test_real_stat_implementation_contract_record_is_declared():
    text = _text()
    required_fields = [
        "REAL_STAT_IMPLEMENTATION_CONTRACT_RECORD_ID",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_INPUT_RECORD_ID",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_SOURCE_READINESS_RECORD_ID",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_SOURCE_STAT_EXECUTION_BOUNDARY_RECORD_ID",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_SOURCE_STAT_EXECUTION_BOUNDARY_HANDLE",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_SOURCE_CONTROLLED_STAT_BOUNDARY_RECORD_ID",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_SOURCE_CONTROLLED_STAT_BOUNDARY_HANDLE",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_SOURCE_REAL_FILE_ACCESS_BOUNDARY_RECORD_ID",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_SOURCE_REAL_FILE_ACCESS_BOUNDARY_HANDLE",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_SOURCE_LOCAL_PATH_DISCLOSURE_BOUNDARY_RECORD_ID",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_SOURCE_LOCAL_PATH_DISCLOSURE_BOUNDARY_HANDLE",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_SOURCE_SELECTION_BOUNDARY_RECORD_ID",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_SOURCE_SELECTION_BOUNDARY_HANDLE",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_SOURCE_CONFIRMATION_RECORD_ID",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_SOURCE_CONFIRMATION_HANDLE",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_SOURCE_CONFIRMATION_VALUE",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_SOURCE_TOKEN_RECORD_ID",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_SOURCE_TOKEN_HANDLE",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_SANITIZED_SELECTION_TOKEN",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_SOURCE_REFERENCE_RECORD_ID",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_SOURCE_REFERENCE_HANDLE",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_SOURCE_EVENT_ID",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_SOURCE_EVENT_HANDLE",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_HANDLE",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_VERDICT",
    ]
    for field in required_fields:
        assert field in text


def test_real_stat_implementation_contract_values_are_sanitized():
    text = _text()
    required_values = [
        "real_stat_implementation_contract_001",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_HANDLE_001",
        "defined_as_isolated_stat_implementation_contract",
        "prepares_isolated_implementation_readiness_only",
        "contract_defined_without_runtime_creation",
        "real_stat_implementation_contract_defined_without_runtime_stat_or_metadata_read",
    ]
    for value in required_values:
        assert value in text


def test_contract_status_and_scope_are_limited():
    text = _text()
    assert "REAL_STAT_IMPLEMENTATION_CONTRACT_STATUS" in text
    assert "defined_as_isolated_stat_implementation_contract" in text
    assert "REAL_STAT_IMPLEMENTATION_CONTRACT_SCOPE_STATUS" in text
    assert "prepares_isolated_implementation_readiness_only" in text
    assert "The contract status is `defined_as_isolated_stat_implementation_contract`." in text
    assert "The contract scope prepares isolated implementation readiness only." in text


def test_no_implementation_or_runtime_is_created():
    text = _text()
    assert "REAL_STAT_IMPLEMENTATION_CONTRACT_IMPLEMENTATION_STATUS" in text
    assert "contract_defined_without_runtime_creation" in text
    assert "REAL_STAT_IMPLEMENTATION_CONTRACT_RUNTIME_STATUS" in text
    assert "no_runtime_created" in text
    assert "No implementation runtime is created." in text


def test_existing_cli_runtime_is_not_modified():
    text = _text()
    assert "REAL_STAT_IMPLEMENTATION_CONTRACT_CLI_RUNTIME_STATUS" in text
    assert "not_modified" in text
    assert "Existing CLI runtime is not modified." in text


def test_no_stat_execution_occurs():
    text = _text()
    assert "REAL_STAT_IMPLEMENTATION_CONTRACT_STAT_STATUS" in text
...
def test_no_real_file_access_occurs():
    text = _text()
    assert "REAL_STAT_IMPLEMENTATION_CONTRACT_ACCESS_STATUS" in text
...
def test_no_media_file_is_opened():
    text = _text()
    assert "REAL_STAT_IMPLEMENTATION_CONTRACT_FILE_OPEN_STATUS" in text
...
def test_no_file_bytes_or_real_metadata_are_read():
    text = _text()
    required_statuses = [
        "REAL_STAT_IMPLEMENTATION_CONTRACT_FILE_BYTES_STATUS",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_FILESYSTEM_METADATA_STATUS",
        "not_read",
    ]
...
def test_no_file_size_timestamps_or_hashes_are_recorded():
    text = _text()
    required_statuses = [
        "REAL_STAT_IMPLEMENTATION_CONTRACT_FILE_SIZE_STATUS",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_TIMESTAMP_STATUS",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_HASH_STATUS",
        "not_recorded",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_local_path_or_identity_metadata_is_committed():
    text = _text()
    required_statuses = [
        "REAL_STAT_IMPLEMENTATION_CONTRACT_REAL_PATH_STATUS",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_ABSOLUTE_PATH_STATUS",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_RELATIVE_PATH_STATUS",
        "not_recorded_in_committed_artifacts",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_REAL_FILENAME_STATUS",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_PARENT_FOLDER_STATUS",
        "not_recorded",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_media_decode_probe_scan_or_transcription_occurs():
    text = _text()
    required_statuses = [
        "REAL_STAT_IMPLEMENTATION_CONTRACT_MEDIA_DECODE_STATUS",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_MEDIA_PROBE_STATUS",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_MEDIA_SCAN_STATUS",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_TRANSCRIPTION_STATUS",
        "not_executed",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_thumbnails_or_waveforms_are_generated():
    text = _text()
    required_statuses = [
        "REAL_STAT_IMPLEMENTATION_CONTRACT_THUMBNAIL_STATUS",
        "not_generated",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_WAVEFORM_STATUS",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_execution_or_media_tools_are_run():
    text = _text()
    required_statuses = [
        "REAL_STAT_IMPLEMENTATION_CONTRACT_EXECUTION_STATUS",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_FFMPEG_STATUS",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_FFPROBE_STATUS",
        "REAL_STAT_IMPLEMENTATION_CONTRACT_SCANNER_STATUS",
        "not_executed",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_saas_integration_is_created():
    text = _text()
    assert "REAL_STAT_IMPLEMENTATION_CONTRACT_SAAS_STATUS" in text
    assert "no_saas_integration" in text
    assert "No SaaS integration is created." in text


def test_isolated_implementation_contract_requirements_are_declared():
    text = _text()
    requirements = [
        "The later implementation must remain isolated from SaaS runtime.",
        "The later implementation must remain isolated from media probing.",
        "The later implementation must remain isolated from scanner execution.",
        "The later implementation must accept only a sanitized control input.",
        "The later implementation must not require a committed local path.",
        "The later implementation must not require a committed sensitive filename.",
        "The later implementation must not require committed parent folder data.",
        "The later implementation must not commit real file size.",
        "The later implementation must not commit real timestamps.",
        "The later implementation must not commit real hashes.",
        "The later implementation must not read file bytes.",
        "The later implementation must not open media.",
        "The later implementation must not run FFmpeg.",
        "The later implementation must not run ffprobe.",
        "The later implementation must not run scanner logic.",
        "The later implementation must not decode media.",
        "The later implementation must not transcribe media.",
        "The later implementation must not generate thumbnails.",
        "The later implementation must not generate waveforms.",
        "The later implementation must return sanitized outputs only.",
        "The later implementation must be covered by isolated tests.",
    ]
...
def test_positive_assertions_preserve_scope():
    ...
    required_assertions = [
        ...
        "`real_stat_implementation_contract_001` is created as a sanitized contract record.",
        "`REAL_STAT_IMPLEMENTATION_CONTRACT_HANDLE_001` is a non-filesystem contract handle.",
        "`real_stat_implementation_readiness_001` remains the source readiness record.",
...
def test_later_isolated_implementation_readiness_constraints_are_conservative():
    text = _text()
    constraints = [
...
def test_next_phase_boundary_is_isolated_implementation_readiness_only():
    text = _text()
    assert "The next conservative phase may prepare an isolated implementation readiness gate." in text
    assert "This real stat implementation gate does not authorize runtime implementation." in text
    assert "This real stat implementation gate does not authorize filesystem stat execution." in text
    assert "This real stat implementation gate does not authorize accessing a real file." in text
    assert "This real stat implementation gate does not authorize opening media." in text
    assert "This real stat implementation gate does not authorize reading file bytes." in text
    assert "This real stat implementation gate does not authorize reading real metadata." in text
    assert "This real stat implementation gate does not authorize media execution." in text
    assert "This real stat implementation gate only defines the isolated real stat implementation contract." in text


def test_required_checks_reference_previous_gates_and_generic_guards():
    text = _text()
    required_checks = [
        "This real stat implementation gate test.",
...
def test_closing_state_is_ready_for_isolated_implementation_readiness_gate():
    text = _text()
    assert "REAL_STAT_IMPLEMENTATION_CONTRACT_READY_FOR_ISOLATED_IMPLEMENTATION_READINESS_GATE" in text
