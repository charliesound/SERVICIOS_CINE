...
DOC = ROOT / "docs/product/local_media_agent/cid_local_media_agent_real_media_preflight_controlled_stat_readiness_gate_v1.md"
...
def test_controlled_stat_readiness_gate_doc_exists():
    assert DOC.exists()
...
def test_starting_state_is_from_real_file_access_gate():
    text = _text()
    assert "REAL_FILE_ACCESS_BOUNDARY_READY_FOR_CONTROLLED_STAT_READINESS_GATE" in text


def test_target_state_is_ready_for_controlled_stat_gate():
    text = _text()
...
def test_gate_purpose_is_readiness_only():
    text = _text()
    assert "This readiness gate prepares the conditions for a later controlled filesystem stat gate." in text
...
def test_source_real_file_access_gate_is_referenced():
    text = _text()
...
def test_source_real_file_access_boundary_record_values_are_preserved():
    text = _text()
    required_values = [
...
def test_controlled_stat_readiness_record_is_declared():
    text = _text()
    required_fields = [
        "CONTROLLED_STAT_READINESS_RECORD_ID",
        "CONTROLLED_STAT_INPUT_RECORD_ID",
        "CONTROLLED_STAT_SOURCE_REAL_FILE_ACCESS_BOUNDARY_RECORD_ID",
        "CONTROLLED_STAT_SOURCE_REAL_FILE_ACCESS_BOUNDARY_HANDLE",
        "CONTROLLED_STAT_SOURCE_LOCAL_PATH_DISCLOSURE_BOUNDARY_RECORD_ID",
        "CONTROLLED_STAT_SOURCE_LOCAL_PATH_DISCLOSURE_BOUNDARY_HANDLE",
        "CONTROLLED_STAT_SOURCE_SELECTION_BOUNDARY_RECORD_ID",
        "CONTROLLED_STAT_SOURCE_SELECTION_BOUNDARY_HANDLE",
        "CONTROLLED_STAT_SOURCE_CONFIRMATION_RECORD_ID",
        "CONTROLLED_STAT_SOURCE_CONFIRMATION_HANDLE",
        "CONTROLLED_STAT_SOURCE_CONFIRMATION_VALUE",
        "CONTROLLED_STAT_SOURCE_TOKEN_RECORD_ID",
        "CONTROLLED_STAT_SOURCE_TOKEN_HANDLE",
        "CONTROLLED_STAT_SANITIZED_SELECTION_TOKEN",
        "CONTROLLED_STAT_SOURCE_REFERENCE_RECORD_ID",
        "CONTROLLED_STAT_SOURCE_REFERENCE_HANDLE",
        "CONTROLLED_STAT_SOURCE_EVENT_ID",
        "CONTROLLED_STAT_SOURCE_EVENT_HANDLE",
        "CONTROLLED_STAT_VERDICT",
    ]
    for field in required_fields:
        assert field in text


def test_controlled_stat_readiness_values_are_sanitized():
    text = _text()
    required_values = [
...
def test_readiness_status_is_ready_for_controlled_stat_gate():
    text = _text()
    assert "CONTROLLED_STAT_READINESS_STATUS" in text
    assert "ready_for_controlled_stat_gate" in text
    assert "The readiness status is `ready_for_controlled_stat_gate`." in text


def test_no_stat_operation_occurs_in_this_gate():
    text = _text()
    assert "CONTROLLED_STAT_STAT_STATUS" in text
...
def test_no_real_file_access_occurs_in_this_gate():
    text = _text()
    assert "CONTROLLED_STAT_ACCESS_STATUS" in text
...
def test_no_media_file_is_opened_in_this_gate():
    text = _text()
    assert "CONTROLLED_STAT_FILE_OPEN_STATUS" in text
...
def test_no_file_bytes_or_real_metadata_are_read():
    text = _text()
    required_statuses = [
        "CONTROLLED_STAT_FILE_BYTES_STATUS",
        "CONTROLLED_STAT_FILESYSTEM_METADATA_STATUS",
...
def test_no_file_size_timestamps_or_hashes_are_recorded():
    text = _text()
    required_statuses = [
        "CONTROLLED_STAT_FILE_SIZE_STATUS",
        "CONTROLLED_STAT_TIMESTAMP_STATUS",
        "CONTROLLED_STAT_HASH_STATUS",
...
def test_no_local_path_or_identity_metadata_is_committed():
    text = _text()
    required_statuses = [
        "CONTROLLED_STAT_REAL_PATH_STATUS",
        "CONTROLLED_STAT_ABSOLUTE_PATH_STATUS",
        "CONTROLLED_STAT_RELATIVE_PATH_STATUS",
        "not_recorded_in_committed_artifacts",
        "CONTROLLED_STAT_REAL_FILENAME_STATUS",
        "CONTROLLED_STAT_PARENT_FOLDER_STATUS",
        "not_recorded_in_this_gate",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_media_decode_probe_scan_or_transcription_occurs():
    text = _text()
    required_statuses = [
        "CONTROLLED_STAT_MEDIA_DECODE_STATUS",
        "CONTROLLED_STAT_MEDIA_PROBE_STATUS",
        "CONTROLLED_STAT_MEDIA_SCAN_STATUS",
        "CONTROLLED_STAT_TRANSCRIPTION_STATUS",
...
def test_no_thumbnails_or_waveforms_are_generated():
    text = _text()
    required_statuses = [
        "CONTROLLED_STAT_THUMBNAIL_STATUS",
        "not_generated_in_this_gate",
        "CONTROLLED_STAT_WAVEFORM_STATUS",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_execution_or_media_tools_are_run():
    text = _text()
    required_statuses = [
        "CONTROLLED_STAT_EXECUTION_STATUS",
        "CONTROLLED_STAT_FFMPEG_STATUS",
        "CONTROLLED_STAT_FFPROBE_STATUS",
        "CONTROLLED_STAT_SCANNER_STATUS",
        "not_executed_in_this_gate",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_runtime_or_saas_integration_is_created():
    text = _text()
    assert "CONTROLLED_STAT_RUNTIME_STATUS" in text
    assert "no_runtime_created" in text
    assert "CONTROLLED_STAT_SAAS_STATUS" in text
...
def test_positive_assertions_preserve_scope():
    ...
    required_assertions = [
        ...
        "`controlled_stat_readiness_001` is created as a readiness record.",
...
def test_later_controlled_stat_constraints_are_conservative():
    text = _text()
    constraints = [
...
def test_next_phase_boundary_is_controlled_stat_gate_only():
    text = _text()
    assert "The next conservative phase may define a controlled stat gate." in text
    assert "This controlled stat readiness gate does not authorize filesystem stat operations." in text
    assert "This controlled stat readiness gate does not authorize accessing a real file." in text
    assert "This controlled stat readiness gate does not authorize opening media." in text
    assert "This controlled stat readiness gate does not authorize reading file bytes." in text
    assert "This controlled stat readiness gate does not authorize reading real metadata." in text
    assert "This controlled stat readiness gate does not authorize media execution." in text
    assert "This controlled stat readiness gate only prepares conditions for a later controlled stat gate." in text


def test_required_checks_reference_previous_gates_and_generic_guards():
    text = _text()
    required_checks = [
        "This controlled stat readiness gate test.",
...
def test_closing_state_is_ready_for_controlled_stat_gate():
    text = _text()
    assert "READY_FOR_CONTROLLED_STAT_GATE" in text
