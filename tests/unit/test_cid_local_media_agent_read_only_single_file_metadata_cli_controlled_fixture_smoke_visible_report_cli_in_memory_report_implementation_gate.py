...
DOC = ROOT / "docs/product/local_media_agent/cid_local_media_agent_read_only_single_file_metadata_cli_controlled_fixture_smoke_visible_report_cli_in_memory_report_implementation_gate_v1.md"
...
PHASE = "CID.LOCAL_MEDIA_AGENT.READ_ONLY.SINGLE_FILE.METADATA.CLI.CONTROLLED_FIXTURE.SMOKE.VISIBLE_REPORT.CLI.IN_MEMORY.REPORT.IMPLEMENTATION.GATE.V1"
RESULT = "LOCAL_MEDIA_AGENT_READ_ONLY_SINGLE_FILE_METADATA_CLI_CONTROLLED_FIXTURE_SMOKE_VISIBLE_REPORT_CLI_IN_MEMORY_REPORT_IMPLEMENTATION_GATE_V1_CLOSED"
BASE_HEAD = "1c77336b8ab5560bb3429e0619aeeb2448fab068"
BASE_TAG = "cid-dev-stable-local-media-agent-read-only-single-file-metadata-cli-controlled-fixture-smoke-visible-report-cli-in-memory-report-readiness-gate-v1-20260701"
EXPECTED_BYTES = 239
...
def load_implementation_module():
    spec = importlib.util.spec_from_file_location("cid_lma_read_only_metadata_visible_report_impl", IMPLEMENTATION)
...
def test_implementation_document_declares_phase_result_base_flag_and_scope() -> None:
    assert DOC.exists()
    body = read_text(DOC)
    required = [
        PHASE,
        ...
        "keeps default CLI status output unchanged",
        "keeps --result-json behavior unchanged",
...
def test_implementation_module_is_parseable_and_parser_exposes_visible_report_flag() -> None:
...
def test_implementation_imports_remain_standard_library_only_in_ast() -> None:
    roots = import_roots(parse_python(IMPLEMENTATION))
    assert roots <= {"__future__", "argparse", "hashlib", "json", "pathlib", "typing"}


def test_implementation_uses_dynamic_internal_integration_without_direct_cli_import() -> None:
    body = read_text(IMPLEMENTATION)
    assert "controlled_fixture_smoke_visible_report_in_memory_integration" in body
    assert "render_controlled_fixture_smoke_result_in_memory" in body
    assert "read_only_single_file_metadata_cli" not in body
    assert "__import__(" in body


def test_implementation_has_no_process_scan_media_probe_or_export_calls() -> None:
    ...
    forbidden_leaf_calls = {
        ...
        "write_text", "write_bytes", "touch", "mkdir", "unlink", "rename", "replace", "remove", "rmdir",
    }
    found: list[str] = []
    for node in ast.walk(tree):
...
def test_default_cli_status_output_remains_unchanged() -> None:
    exit_code, stdout = run_cli_and_capture(base_argv())
    assert exit_code == 0
    assert stdout == "CONTROLLED_READ_ONLY_SINGLE_FILE_METADATA_OK\n"


def test_result_json_output_remains_json_and_unchanged() -> None:
...
def test_visible_report_markdown_output_is_explicit_and_in_memory() -> None:
    exit_code, stdout = run_cli_and_capture(base_argv() + ["--visible-report-markdown"])
    assert exit_code == 0
    ...
    assert stdout.rstrip() != "CONTROLLED_READ_ONLY_SINGLE_FILE_METADATA_OK"


def test_visible_report_markdown_preserves_rejected_exit_code() -> None:
    exit_code, stdout = run_cli_and_capture(base_argv()[:-6] + ["--expected-sha256", "0" * 64, "--expected-bytes", str(EXPECTED_BYTES), "--allowed-relative-path", ALLOWED_RELATIVE_PATH, "--visible-report-markdown"])
    assert exit_code == 2
    assert "# CID Local Media Agent - Controlled Fixture Smoke Visible Report" in stdout
    assert "FAIL" in stdout
    assert "2" in stdout


def test_wrapper_artifact_remains_parseable_and_delegating() -> None:
    assert WRAPPER.exists()
    tree = parse_python(WRAPPER)
    assert isinstance(tree, ast.Module)
    wrapper_body = read_text(WRAPPER)
    assert "implementation.run_cli(argv)" in wrapper_body
    assert FUTURE_FLAG not in wrapper_body
...
def test_pyproject_does_not_register_visible_report_mode() -> None:
