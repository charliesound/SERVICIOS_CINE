...
DOC = ROOT / "docs/product/local_media_agent/cid_local_media_agent_real_media_preflight_controlled_stat_gate_v1.md"
...
def test_controlled_stat_gate_doc_exists():
    assert DOC.exists()


def test_phase_identifier_is_present():
    text = _text()
    assert "CID.LOCAL_MEDIA_AGENT.REAL_MEDIA_PREFLIGHT.CONTROLLED_STAT.GATE.V1" in text


def test_expected_closure_result_is_present():
    text = _text()
    assert "LOCAL_MEDIA_AGENT_REAL_MEDIA_PREFLIGHT_CONTROLLED_STAT_GATE_V1_CLOSED" in text


def test_starting_state_is_ready_for_controlled_stat_gate():
    text = _text()
    assert "READY_FOR_CONTROLLED_STAT_GATE" in text


def test_target_state_prepares_stat_execution_readiness():
    text = _text()
    assert "CONTROLLED_STAT_BOUNDARY_READY_FOR_STAT_EXECUTION_READINESS_GATE" in text


def test_gate_purpose_defines_boundary_only():
    text = _text()
    assert "This gate defines the controlled stat boundary for a later stat execution readiness phase." in text
    assert "This gate creates only a sanitized controlled stat boundary record." in text
...
def test_source_controlled_stat_readiness_gate_is_referenced():
    text = _text()
    assert "CID.LOCAL_MEDIA_AGENT.REAL_MEDIA_PREFLIGHT.CONTROLLED_STAT.READINESS.GATE.V1" in text
    assert "LOCAL_MEDIA_AGENT_REAL_MEDIA_PREFLIGHT_CONTROLLED_STAT_READINESS_GATE_V1_CLOSED" in text


def test_source_controlled_stat_readiness_record_values_are_preserved():
    text = _text()
    required_values = [
        "controlled_stat_readiness_001",
        "operator_input_001",
        ...
        "single_file_claimed",
        "ready_for_controlled_stat_gate_without_stat_open_or_metadata_read",
    ]
    for value in required_values:
        assert value in text


def test_controlled_stat_boundary_record_is_declared():
    text = _text()
    required_fields = [
        "CONTROLLED_STAT_BOUNDARY_RECORD_ID",
        "CONTROLLED_STAT_BOUNDARY_INPUT_RECORD_ID",
        "CONTROLLED_STAT_BOUNDARY_SOURCE_READINESS_RECORD_ID",
        "CONTROLLED_STAT_BOUNDARY_SOURCE_REAL_FILE_ACCESS_BOUNDARY_RECORD_ID",
        "CONTROLLED_STAT_BOUNDARY_SOURCE_REAL_FILE_ACCESS_BOUNDARY_HANDLE",
        "CONTROLLED_STAT_BOUNDARY_SOURCE_LOCAL_PATH_DISCLOSURE_BOUNDARY_RECORD_ID",
        "CONTROLLED_STAT_BOUNDARY_SOURCE_LOCAL_PATH_DISCLOSURE_BOUNDARY_HANDLE",
        "CONTROLLED_STAT_BOUNDARY_SOURCE_SELECTION_BOUNDARY_RECORD_ID",
        "CONTROLLED_STAT_BOUNDARY_SOURCE_SELECTION_BOUNDARY_HANDLE",
        "CONTROLLED_STAT_BOUNDARY_SOURCE_CONFIRMATION_RECORD_ID",
        "CONTROLLED_STAT_BOUNDARY_SOURCE_CONFIRMATION_HANDLE",
        "CONTROLLED_STAT_BOUNDARY_SOURCE_CONFIRMATION_VALUE",
        "CONTROLLED_STAT_BOUNDARY_SOURCE_TOKEN_RECORD_ID",
        "CONTROLLED_STAT_BOUNDARY_SOURCE_TOKEN_HANDLE",
        "CONTROLLED_STAT_BOUNDARY_SANITIZED_SELECTION_TOKEN",
        "CONTROLLED_STAT_BOUNDARY_SOURCE_REFERENCE_RECORD_ID",
        "CONTROLLED_STAT_BOUNDARY_SOURCE_REFERENCE_HANDLE",
        "CONTROLLED_STAT_BOUNDARY_SOURCE_EVENT_ID",
        "CONTROLLED_STAT_BOUNDARY_SOURCE_EVENT_HANDLE",
        "CONTROLLED_STAT_BOUNDARY_HANDLE",
        "CONTROLLED_STAT_BOUNDARY_VERDICT",
    ]
    for field in required_fields:
        assert field in text


def test_controlled_stat_boundary_values_are_sanitized():
    text = _text()
    required_values = [
        "controlled_stat_boundary_001",
        "CONTROLLED_STAT_BOUNDARY_HANDLE_001",
        "defined_as_operator_local_stat_boundary",
        "prepares_stat_execution_readiness_only",
        "boundary_defined_without_stat_execution",
        "controlled_stat_boundary_defined_without_stat_execution_or_metadata_read",
    ]
    for value in required_values:
        assert value in text
...
def test_boundary_status_and_scope_are_limited():
    text = _text()
    assert "CONTROLLED_STAT_BOUNDARY_STATUS" in text
    assert "defined_as_operator_local_stat_boundary" in text
    assert "CONTROLLED_STAT_BOUNDARY_SCOPE_STATUS" in text
    assert "prepares_stat_execution_readiness_only" in text
    assert "The boundary status is `defined_as_operator_local_stat_boundary`." in text
    assert "The boundary scope prepares stat execution readiness only." in text


def test_no_stat_operation_occurs():
    text = _text()
    assert "CONTROLLED_STAT_BOUNDARY_STAT_STATUS" in text
...
def test_no_real_file_access_occurs():
    text = _text()
    assert "CONTROLLED_STAT_BOUNDARY_ACCESS_STATUS" in text
...
def test_no_media_file_is_opened():
    text = _text()
    assert "CONTROLLED_STAT_BOUNDARY_FILE_OPEN_STATUS" in text
...
def test_no_file_bytes_or_real_metadata_are_read():
    text = _text()
    required_statuses = [
        "CONTROLLED_STAT_BOUNDARY_FILE_BYTES_STATUS",
        "CONTROLLED_STAT_BOUNDARY_FILESYSTEM_METADATA_STATUS",
        "not_read",
    ]
...
def test_no_file_size_timestamps_or_hashes_are_recorded():
    text = _text()
    required_statuses = [
        "CONTROLLED_STAT_BOUNDARY_FILE_SIZE_STATUS",
        "CONTROLLED_STAT_BOUNDARY_TIMESTAMP_STATUS",
        "CONTROLLED_STAT_BOUNDARY_HASH_STATUS",
        "not_recorded",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_local_path_or_identity_metadata_is_committed():
    text = _text()
    required_statuses = [
        "CONTROLLED_STAT_BOUNDARY_REAL_PATH_STATUS",
        "CONTROLLED_STAT_BOUNDARY_ABSOLUTE_PATH_STATUS",
        "CONTROLLED_STAT_BOUNDARY_RELATIVE_PATH_STATUS",
        "not_recorded_in_committed_artifacts",
        "CONTROLLED_STAT_BOUNDARY_REAL_FILENAME_STATUS",
        "CONTROLLED_STAT_BOUNDARY_PARENT_FOLDER_STATUS",
        "not_recorded",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_media_decode_probe_scan_or_transcription_occurs():
    text = _text()
    required_statuses = [
        "CONTROLLED_STAT_BOUNDARY_MEDIA_DECODE_STATUS",
        "CONTROLLED_STAT_BOUNDARY_MEDIA_PROBE_STATUS",
        "CONTROLLED_STAT_BOUNDARY_MEDIA_SCAN_STATUS",
        "CONTROLLED_STAT_BOUNDARY_TRANSCRIPTION_STATUS",
        "not_executed",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_thumbnails_or_waveforms_are_generated():
    text = _text()
    required_statuses = [
        "CONTROLLED_STAT_BOUNDARY_THUMBNAIL_STATUS",
        "not_generated",
        "CONTROLLED_STAT_BOUNDARY_WAVEFORM_STATUS",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_execution_or_media_tools_are_run():
    text = _text()
    required_statuses = [
        "CONTROLLED_STAT_BOUNDARY_EXECUTION_STATUS",
        "CONTROLLED_STAT_BOUNDARY_FFMPEG_STATUS",
        "CONTROLLED_STAT_BOUNDARY_FFPROBE_STATUS",
        "CONTROLLED_STAT_BOUNDARY_SCANNER_STATUS",
        "not_executed",
    ]
    for status in required_statuses:
        assert status in text
...
def test_no_runtime_or_saas_integration_is_created():
    text = _text()
    assert "CONTROLLED_STAT_BOUNDARY_RUNTIME_STATUS" in text
    assert "no_runtime_created" in text
    assert "CONTROLLED_STAT_BOUNDARY_SAAS_STATUS" in text
...
def test_positive_assertions_preserve_scope():
    ...
    required_assertions = [
        ...
        "`controlled_stat_boundary_001` is created as a sanitized boundary record.",
        "`CONTROLLED_STAT_BOUNDARY_HANDLE_001` is a non-filesystem boundary handle.",
        "`controlled_stat_readiness_001` remains the source readiness record.",
...
def test_later_stat_execution_readiness_constraints_are_conservative():
    text = _text()
    constraints = [
...
def test_controlled_stat_and_file_access_are_forbidden():
    text = _text()
    forbidden_items = [
...
def test_next_phase_boundary_is_stat_execution_readiness_only():
    text = _text()
    assert "The next conservative phase may prepare a stat execution readiness gate." in text
    assert "This controlled stat gate does not authorize filesystem stat operations." in text
    assert "This controlled stat gate does not authorize accessing a real file." in text
    assert "This controlled stat gate does not authorize opening media." in text
    assert "This controlled stat gate does not authorize reading file bytes." in text
    assert "This controlled stat gate does not authorize reading real metadata." in text
    assert "This controlled stat gate does not authorize media execution." in text
    assert "This controlled stat gate only defines the sanitized controlled stat boundary." in text


def test_required_checks_reference_previous_gates_and_generic_guards():
    text = _text()
    required_checks = [
        "This controlled stat gate test.",
...
def test_closing_state_is_ready_for_stat_execution_readiness_gate():
    text = _text()
    assert "CONTROLLED_STAT_BOUNDARY_READY_FOR_STAT_EXECUTION_READINESS_GATE" in text
