...
QA_DOC = Path(
    ...
    "cid_local_media_agent_ffprobe_controlled_file_metadata_visible_report_renderer_cli_visible_report_output_export_controlled_text_artifact_smoke_controlled_fixture_contract_qa_gate_v1.md"
)
CONTRACT_DOC = Path(
    "docs/product/local_media_agent/"
...
FUNCTIONAL_RESULT = (
    ...
    "VISIBLE_REPORT_OUTPUT_EXPORT_CONTROLLED_TEXT_ARTIFACT_SMOKE_CONTROLLED_FIXTURE_CONTRACT_QA_GATE_PASS_CLOSED"
)
TARGET_TAG = (
    "cid-dev-stable-local-media-agent-ffprobe-controlled-file-metadata-visible-report-"
    "renderer-cli-visible-report-output-export-controlled-text-artifact-smoke-controlled-fixture-contract-qa-gate-v1-20260622"
)
NEXT_PHASE = (
    "CID.LOCAL_MEDIA_AGENT.FFPROBE.CONTROLLED.FILE.METADATA.PREFLIGHT."
    "VISIBLE.REPORT.RENDERER.CLI.VISIBLE.REPORT.OUTPUT.EXPORT.CONTROLLED.TEXT.ARTIFACT.SMOKE.CONTROLLED.FIXTURE.IMPLEMENTATION.READINESS.CONTRACT.V1"
)
...
def test_qa_gate_declares_no_export_no_packaging_no_files_no_new_fixtures() -> None:
    assert_all_present(read(QA_DOC), [
...
def test_qa_gate_declares_validated_definition() -> None:
    assert_all_present(read(QA_DOC), [
        "## Validated Smoke Controlled Fixture Definition",
...
def test_qa_gate_declares_validated_future_constraints() -> None:
    assert_all_present(read(QA_DOC), [
        "## Validated Future Smoke Fixture Constraints",
...
def test_contract_doc_declares_smoke_controlled_fixture_definition() -> None:
    assert_all_present(read(CONTRACT_DOC), [
        "document-only smoke fixture contract",
        "existing controlled synthetic JSON fixture only",
        "existing expected visible text fixture",
...
def test_contract_doc_references_required_artifacts() -> None:
    ...
    contract_own_test = Path(
        "tests/unit/"
        "test_cid_local_media_agent_ffprobe_controlled_file_metadata_visible_report_renderer_cli_visible_report_output_export_controlled_text_artifact_smoke_controlled_fixture_contract.py"
    )
    for artifact in REQUIRED_ARTIFACTS:
        if artifact in {CONTRACT_DOC, contract_own_test}:
            continue
