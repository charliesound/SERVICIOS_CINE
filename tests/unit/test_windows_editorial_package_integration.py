...
def assembled_package(tmp_path: Path):
    ...
    """
    pkg = tmp_path / "CID-Local-Media-Agent-0.3.0-beta5"
...
# ---------------- beta5 version / installed manifest -----------------

def test_package_version_is_beta5() -> None:
    assert B.VERSION == "0.3.0-beta5"
    assert B.PACKAGE_NAME == "CID-Local-Media-Agent-0.3.0-beta5"


def test_gui_app_version_is_beta5() -> None:
    from scripts.local_media_agent.cid_gui import APP_VERSION

    assert APP_VERSION == "0.3.0-beta5"
