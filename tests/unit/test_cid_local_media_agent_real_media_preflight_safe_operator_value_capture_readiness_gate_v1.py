...
def test_value_capture_readiness_declares_phase_result_and_status() -> None:
    text = _text()
...
def test_value_capture_readiness_records_base_state() -> None:
    text = _text()

    assert "0dc676fba2b0a8c5615bb7b8fe6f0be9e695cb05" in text
    assert "0dc676f docs: add CID Local Media Agent operator sanitized candidate input gate" in text
    assert "cid-dev-stable-local-media-agent-real-media-preflight-operator-sanitized-candidate-input-gate-v1-20260702" in text


def test_value_capture_readiness_references_upstream_documents() -> None:
    text = _text()
...
def test_value_capture_readiness_is_readiness_only() -> None:
    text = _text()
...
def test_value_capture_readiness_explains_why_gate_exists() -> None:
    text = _text()
...
def test_value_capture_readiness_records_core_fields() -> None:
    text = _text()
...
def test_value_capture_readiness_records_no_input_candidate_file_or_dependency() -> None:
    text = _text()
...
def test_value_capture_readiness_lists_required_capture_fields() -> None:
    text = _text()
...
def test_value_capture_readiness_defines_allowed_values() -> None:
    text = _text()
...
def test_value_capture_readiness_blocks_forbidden_values() -> None:
    text = _text()
...
def test_value_capture_readiness_defines_redaction_and_review_rules() -> None:
    text = _text()
...
def test_value_capture_readiness_lists_stop_conditions() -> None:
    text = _text()
...
def test_value_capture_readiness_lists_future_pass_criteria() -> None:
    text = _text()
...
def test_value_capture_readiness_defines_supported_and_unsupported_scope() -> None:
    text = _text()
...
def test_value_capture_readiness_records_pass_criteria() -> None:
    text = _text()
...
def test_value_capture_readiness_keeps_limitations_and_safety_active() -> None:
    text = _text()
...
def test_value_capture_readiness_keeps_allowed_and_forbidden_scope_explicit() -> None:
    text = _text()
...
def test_value_capture_readiness_has_next_phase_commit_and_tag_guidance() -> None:
    text = _text()
    ...
    assert "docs: add CID Local Media Agent safe operator value capture readiness gate" in text
